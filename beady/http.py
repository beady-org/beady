...
r"""\
Beady HTTP layer / WSGI application
...
:class:`~beady.http.Controller`: class and its companion the
:func:`~beady.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Beady http request. The Beady http request is
...
Beady URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.beady.org/documentation/master/developer/reference/beads/http.html#csrf
for more details.

* if this endpoint is accessed through Beady via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~beady.service.db.list_dbs`:
    ...
    """
    try:
        dbs = beady.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Beady will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': beady.service.common.dispatch,
        'db': beady.service.db.dispatch,
        'object': beady.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~beady.tools.file_path`
        :param filter_ext: See :func:`~beady.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`beady.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~beady.http.Controller` can use the :func:`~beady.http.route`:
    ...
    :func:`~beady.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(beady.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['beady', 'beads'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~beady.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['beady', 'beads'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Beady beads.
        """
        # Controllers defined outside of beady beads are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of beady beads can be extended in
        # other installed beads. Rebuild the class inheritance here.
        highest_controllers = []
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = beady.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = beady.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~beady.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> beady_ip = socket.gethostbyname(.beady.org')
        >>> GeoIP(beady_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~beady.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~beady.beads.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~beady.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = beady.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to beady.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~beady.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Beady Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for beads_path in beady.beads.__path__:
            for module in os.listdir(beads_path):
                manifest = get_manifest(module)
                static_path = opj(beads_path, module, 'static')
                if (manifest
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + beady.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = beady.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if beady.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/beady/')
                            or httprequest.path in (
                                '/beady', '/web', '/web/login', '/test_http/ensure_db',
                            )):
