...
import beady
from beady.modules import get_modules
from beady.modules.registry import Registry
from beady.release import nt_service_name
from beady.tools import config
from beady.tools.cache import log_ormcache_stats
from beady.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if beady.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'beady.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in beady.beads.__path__:
            _logger.info('Watching beads folder %s', path)
...
class FSWatcherInotify(FSWatcherBase):
    def __init__(self):
        self.started = False
        # ignore warnings from inotify in case we have duplicate beads paths.
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="beady.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from beady.beads.base.models.ir_cron import ir_cron
        conn = beady.sql_db.db_connect('postgres')
        with conn.cursor() as cr:
            ...
            while True:
                ...
                registries = beady.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(beady.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="beady.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from beady.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('beady: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(beady.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = beady.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = beady.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(beady.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            beady.modules.module.load_beady_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the beads found in the `beady-web` project.
Maybe you forgot to add those beads in your beads_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute beady-server process with (nearly) the same arguments"""
    if beady.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from beady.tests import loader  # noqa: PLC0415
    from beady.tests.suite import BeadySuite  # noqa: PLC0415
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from beady.tests import loader  # noqa: PLC0415
                t0 = time.time()
                t0_sql = beady.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = beady.api.Environment(cr, beady.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             beady.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the beady http server and cron processor.
    """
    global server
    ...
    if beady.evented:
        server = GeventServer(beady.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(beady.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Beady is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(beady.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not beady.evented:
        if inotify:
