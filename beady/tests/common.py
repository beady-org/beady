# -*- coding: utf-8 -*-
"""
The module :mod:`beady.tests.common` provides unittest test cases and a few
...
import beady
from beady import api
from beady.exceptions import AccessError
from beady.fields import Command
from beady.modules.registry import Registry
from beady.service import security
from beady.sql_db import BaseCursor, Cursor
from beady.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from beady.tools.mail import single_email_re
from beady.tools.misc import find_in_path, lower_logging
from beady.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: beady.tests.common.Form is deprecated, use beady.tests.Form",
...
# The beady library is supposed already configured.
ADDONS_PATH = beady.tools.config['beads_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = beady.SUPERUSER_ID
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Beady module name.
    """
    def register(func):
        # register func by beady module name
        if func.__module__.startswith('beady.beads.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most beady operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('beady.beads.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Beady-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.beady.org/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~beady.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(beady.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            beady.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = beady.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('beady.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/beady/beads/" in filename:
                        filename = filename.rsplit("/beady/beads/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: beady.models.BaseModel,
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | beady.beads.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(beady.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, beady.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, beady.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('beady.beads.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(beady.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_beady')

        otc = beady.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Beady widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if beady.service.server.server is None:
            return None
        return beady.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('beady.service.http.request.')]
        ...
        if request_threads:
            ...
            beady.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        beady.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            beady.http.root.session_store.delete(self.session)

        self.session = session = beady.http.root.session_store.new()
        session.update(beady.http.get_default_session(), db=get_db_name())
        session.context['lang'] = beady.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('beady.beads.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
        ...
        beady.http.root.session_store.save(session)
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('beady.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from beady.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Beady test suites
