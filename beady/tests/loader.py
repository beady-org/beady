...
from .suite import BeadySuite
from .result import BeadyTestResult
...
def get_test_modules(module):
    """ Return a list of module for the beads potentially containing tests to
    feed get_module_test_cases() """
    results = _get_tests_modules(importlib.util.find_spec(f'beady.beads.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"beady.upgrade.{module}",
        f"beady.beads.{module}.migrations",
        f"beady.beads.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return BeadySuite(sorted(tests, key=lambda t: t.test_sequence))
