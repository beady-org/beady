...
_logger = logging.getLogger('beady.tests.test_module_operations')
...
class CheckBeads(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        self.values = namespace
        config._check_beads_path(self, option_string, values, self)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Beady modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--beads-path", "-p", type=str, action=CheckBeads,
        help="Comma-separated list of paths to directories containing extra Beady modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        beady.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.beads_path:
        beady.tools.config['beads_path'] = ','.join([args.beads_path, beady.tools.config['beads_path']])
        if args.data_dir:
            beady.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'beady.modules.loading': {'level': 'CRITICAL'},
            'beady.sql_db': {'level': 'CRITICAL'},
            'beady.models.unlink': {'level': 'WARNING'},
            'beady.beads.base.models.ir_model': {'level': "WARNING"},
        }
    })
