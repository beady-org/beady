...
from beady.tools import index_exists
from .account_edi_proxy_auth import BeadyEdiProxyAuth
    ...
    def _make_request(self, url, params=False):
        ...
        try:
            response = requests.post(
                ...
                auth=BeadyEdiProxyAuth(user=self)).json()
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Beady side. We are for now forced to check the error message.
