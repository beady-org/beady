...
import { getRangePosition } from '@web_editor/js/editor/beady-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { closestScrollableY } from "@web/core/utils/scrolling";
import { useThrottleForAnimation } from "@web/core/utils/timing";
import { onWillUnmount, onWillStart, reactive, status, useSubEnv } from "@beady/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges(...args) {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.beadyEditor.historyPauseSteps();
            ...
            this.wysiwyg.beadyEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.beadyEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.beadyEditor.historyUnpauseSteps();
            this.wysiwyg.beadyEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.beady-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.beadyEditor.observerFlush();
        this.wysiwyg.beadyEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Beady editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();
                // Update undo/redo buttons
                this.wysiwyg.beadyEditor.dispatchEvent(new Event('historyStep'));

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.beadyEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.beadyEditor.editable.querySelector('p');
                if (p && selection) {
        ...
        if (editableAreaIsEmpty) {
            ...
            // We actually hide the BeadyEditor toolbar by calling
        ...
        this.wysiwyg.beadyEditor.activateContenteditable();
    }
    _getCodeViewEl() {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.beadyEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.beadyEditor.historyStep(true);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await beady.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
