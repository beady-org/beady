...
import beady
from beady.http import Session
from beady.beads.base.tests.common import HttpCaseWithUserDemo
from beady.tools.func import lazy_property
from beady.beads.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(beady.http.Application, 'session_store', session_store)
        cls.classPatch(beady.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(beady.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        beady.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('beady.http.db_list') as db_list, \
             patch('beady.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('beady.http.db_list') as db_list, \
             patch('beady.http.db_filter') as db_filter, \
             patch('beady.http.Registry') as Registry:
