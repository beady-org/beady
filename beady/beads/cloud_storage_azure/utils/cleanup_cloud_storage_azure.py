...
# beady
beady_url = 'http://localhost:8069'
beady_db = 'beady_db'
beady_username = 'admin'
beady_password = 'admin'
...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{beady_url}/xmlrpc/2/common')
    uid = common.authenticate(beady_db, beady_username, beady_password, {})
    models = xmlrpc.client.ServerProxy(f'{beady_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(beady_db, uid, beady_password, 'ir.attachment', 'search_read', [
