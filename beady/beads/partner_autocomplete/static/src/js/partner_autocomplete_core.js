...
export function usePartnerAutocomplete() {
    const keepLastBeady = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        let beadySuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const beadyPromise = getBeadySuggestions(value, isVAT).then((suggestions) => {
                beadySuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with Beady result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = beadySuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            beadySuggestions.push(suggestion);
                        }
                    });
                }

                beadySuggestions = beadySuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                beadySuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(beadySuggestions);
            };

            whenAll([beadyPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use Beady Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getBeadySuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastBeady.add(prom);
