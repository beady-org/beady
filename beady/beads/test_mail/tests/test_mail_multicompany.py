...
from beady.beads.test_mail.models.test_mail_corner_case_models import MailTestMultiCompanyWithActivity
from beady.beads.test_mail.tests.common import TestRecipients
from beady.exceptions import AccessError
from beady.tests import tagged, users, HttpCase
...
class TestMailMCCommon(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.fake_email = cls.env['mail.message'].create({
            ...
            'message_id': '<123456-beady-%s-mail.test.gateway@%s>' % (cls.test_record.id, socket.gethostname()),
        })
...
class TestMultiCompanySetup(TestMailMCCommon, HttpCase):
    ...
    @users('employee_c2')
    @mute_logger('beady.beads.base.models.ir_rule')
            ...
            with self.subTest(login=login, mc_record=mc_record):
                ...
                if not login:
                    ...
                    if mc_error:
                        ...
                        self.assertEqual(parsed_url.path, '/beady/action-mail.action_discuss')
                    else:
