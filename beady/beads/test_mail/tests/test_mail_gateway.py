...
from beady import exceptions
from beady.beads.mail.models.mail_message import Message
from beady.beads.mail.models.mail_thread import MailThread
from beady.beads.mail.tests.common import mail_new_test_user, MailCommon
from beady.beads.test_mail.data import test_mail_data
from beady.beads.test_mail.data.test_mail_data import MAIL_TEMPLATE, THAI_EMAIL_WINDOWS_874
from beady.beads.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups, MailTestTicket
from beady.sql_db import Cursor
from beady.tests import tagged, RecordCapturer
from beady.tools import mute_logger
from beady.tools.mail import email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('beady.beads.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:beady.beads.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml_bounce_headers(self):
        ...
        msg_id = '<861878175823148.1577183525.736005783081055-beady-19177-account.invoice@mycompany.example.com>'
        mail = self.format(
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        message = self.env['mail.message'].create({
            'message_id' : '<368396033905967.1673346177.695352554321289-beady-11-sale.order@eupp00>'
        })
        incoming_bounce = self.format(
            test_mail_data.MAIL_BOUNCE_QP_RFC822_HEADERS,
            email_from='MAILER-DAEMON@mailserver.beady.org (Mail Delivery System)',
            email_to='bounce@xxx.beady.org',
            delivered_to='bounce@xxx.beady.org'
        )
...
class MailGatewayCommon(MailCommon):
    ...
    def _reinject(self, force_msg_id=False, debug_log=False):
        ...
        if mail["headers"].get("X-Beady-Message-Id"):
            extra += f'\nX-Beady-Message-Id: {mail["headers"]["X-Beady-Message-Id"]}'
    ...
    def _create_gateway_message(cls, record, msg_id_prefix, **values):
        msg_values = {
            ...
            'message_id': f'<{msg_id_prefix}-beady-{record.id}-{record._name}@{socket.gethostname()}>',
            'message_type': 'email',
            'model': record._name,
            'res_id': record.id,
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.models')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not beadybot:
        ...
        beadybot.active = True
        beadybot.email = 'beadybot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, beadybot.email_formatted, f'groups@{self.alias_domain}',
                subject='Beadybot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, beadybot)
        self.assertEqual(record4.message_ids[0].email_from, beadybot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: beadybot -> no follower')
    ...
    @mute_logger('beady.beads.mail.models.mail_mail', 'beady.beads.mail.models.mail_thread', 'beady.models.unlink', 'beady.tests')
    ...
    @mute_logger('beady.beads.mail.models.mail_mail', 'beady.beads.mail.models.mail_thread', 'beady.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on beadybot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Beadybot should be creating the message')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.models.unlink', 'beady.beads.mail.models.mail_mail')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.beads.mail.models.mail_mail', 'beady.models.unlink')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.beads.mail.models.mail_mail', 'beady.models', 'beady.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('beady.beads.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('beady.beads.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.models', 'beady.tests')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.models.unlink')
    ...
    @mute_logger('beady.beads.mail.models.mail_thread', 'beady.models.unlink', 'beady.beads.mail.models.mail_mail', 'beady.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by beadybot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but beadybot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by beady should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        beady should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since Beady saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process, MAIL_TEMPLATE,
                          self.partner_1.email_formatted, f'noone@{self.alias_domain}', subject='spam',
                          extra=f'In-Reply-To: <12321321-beady-{self.test_record.id}-{self.test_record._name}@{socket.gethostname()}>')

        # when 6.1 messages are present, compat mode is available
        # Beady 10 update: compat mode has been removed and should not work anymore
        ...
        self.assertRaises(
            ...
            self.format_and_process, MAIL_TEMPLATE,
            self.partner_1.email_formatted, f'noone@{self.alias_domain}>', subject='spam',
            extra=f'In-Reply-To: <12321321-beady-{self.test_record.id}-mail.test.gateway@{socket.gethostname()}>')
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Beady is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('beady.beads.mail.models.mail_mail', 'beady.beads.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Beady servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Beady server -> msg_id is
