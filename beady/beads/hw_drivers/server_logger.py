...
IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in beady.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, beady_server_url, active):
        """
        :param beady_server_url: Beady Server URL
        """
        super().__init__()
        self._beady_server_url = beady_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._beady_server_url:
    ...
    def _periodic_flush(self):
        beady_session = requests.Session()
        while self._beady_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(beady_session)

    def _flush_logs(self, beady_session):
        ...
        if not self._beady_server_url or queue_size == 0:
            return
        try:
            beady_session.post(
                self._beady_server_url + '/iot/log',
                data=empty_queue(),
...
def get_beady_config_log_to_server_option():
    return helpers.get_conf(IOT_LOG_TO_SERVER_CONFIG_NAME, section='options') or True  # Enabled by default


def check_and_update_beady_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_beady_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_beady_server_url(), get_beady_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including beady) is a child
