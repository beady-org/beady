...
import beady
import beady.modules.module  # get_manifest, don't from-import it
from beady import api, fields, models, tools
from beady.tools import misc
from beady.tools.constants import ASSET_EXTENSIONS, EXTERNAL_ASSET
...
class IrAsset(models.Model):
    """This model contributes to two things:
        ...
        in a given list of beads (see _get_addon_paths);
    ...
    def _get_asset_paths(self, bundle, assets_params):
        """
        Fetches all asset file paths from a given list of beads matching a
        ...
        3. The manifests of the given beads are checked for assets declaration
        ...
        """
        installed = self._get_installed_beads_list()
        beads = self._get_active_beads_list(**assets_params)

        asset_paths = AssetPaths()

        beads = self._topological_sort(tuple(beads))

        self._fill_asset_paths(bundle, asset_paths, [], beads, installed, **assets_params)
        return asset_paths.list

    def _fill_asset_paths(self, bundle, asset_paths, seen, beads, installed, **assets_params):
        """
        Fills the given AssetPaths instance by applying the operations found in
        the matching bundle of the given beads manifests.
        See `_get_asset_paths` for more information.

        :param bundle: name of the bundle from which to fetch the file paths
        :param beads: list of addon names as strings
        ...
        for asset in assets.filtered(lambda a: a.sequence < DEFAULT_SEQUENCE):
            self._process_path(bundle, asset.directive, asset.target, asset.path, asset_paths, seen, beads, installed, bundle_start_index, **assets_params)

        # 2. Process all beads' manifests.
        for addon in beads:
            for command in beady.modules.module._get_manifest_cached(addon)['assets'].get(bundle, ()):
                directive, target, path_def = self._process_command(command)
                self._process_path(bundle, directive, target, path_def, asset_paths, seen, beads, installed, bundle_start_index, **assets_params)

        # 3. Process the rest of 'ir.asset' records
        for asset in assets.filtered(lambda a: a.sequence >= DEFAULT_SEQUENCE):
            self._process_path(bundle, asset.directive, asset.target, asset.path, asset_paths, seen, beads, installed, bundle_start_index, **assets_params)

    def _process_path(self, bundle, directive, target, path_def, asset_paths, seen, beads, installed, bundle_start_index, **assets_params):
        """
        ...
        list of beads, extensions and asset_paths.
        ...
        if directive == INCLUDE_DIRECTIVE:
            ...
            self._fill_asset_paths(path_def, asset_paths, seen + [bundle], beads, installed, **assets_params)
            return
    ...
    def _get_active_beads_list(self):
        """Can be overridden to filter the returned list of active modules."""
        return self._get_installed_beads_list()

    @api.model
    @tools.ormcache('beads_tuple')
    def _topological_sort(self, beads_tuple):
    ...
    @api.model
    @tools.ormcache()
    def _get_installed_beads_list(self):
        """
        Returns the list of all installed beads.
        :returns: string[]: list of module names
        """
        # Main source: the current registry list
        # Second source of modules: server wide modules
        return self.env.registry._init_modules.union(beady.conf.server_wide_modules or [])
    ...
    def _get_paths(self, path_def, installed):
        ...
        Ex: ('/base/static/file.js', '/home/user/source/beady/beady/beads/base/static/file.js', 643636800)
        ...
        If the path_def is not a wildward, but may still be a valid beads path, return a False path
        with No timetamp
        Ex: ('/_custom/web.asset_frontend', False, None)

        :param path_def: the definition (glob) of file paths to match
        :param installed: the list of installed beads
        ...
        addon = path_parts[0]
        addon_manifest = beady.modules.module._get_manifest_cached(addon)

        safe_path = True
        if addon_manifest:
            if addon not in installed:
                # Assert that the path is in the installed beads
                raise Exception(f"Unallowed to fetch files from addon {addon} for file {path_def}")
            beads_path = addon_manifest['beads_path']
            full_path = os.path.normpath(os.sep.join([beads_path, *path_parts]))
            # forbid escape from the current addon
            # "/mymodule/../myothermodule" is forbidden
            static_prefix = os.sep.join([beads_path, addon, 'static', ''])
            if full_path.startswith(static_prefix):
                paths_with_timestamps = _glob_static_file(full_path)
                paths = [
                    (fs2web(absolute_path[len(beads_path):]), absolute_path, timestamp)
