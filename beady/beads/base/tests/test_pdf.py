...
from beady.tools import pdf
from beady.tools.misc import file_open
from beady.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.BeadyPdfFileReader(self.minimal_reader_buffer)

    def test_beady_pdf_file_reader(self):
        ...
        r = pdf.BeadyPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_beady_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.BeadyPdfFileWriter()
    ...
    def test_beady_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.BeadyPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.BeadyPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.BeadyPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Beady')
        self.assertEqual(pdf_info['/Creator'], 'Beady')
        reader_buffer.close()
