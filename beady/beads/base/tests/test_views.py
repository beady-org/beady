...
from beady.exceptions import AccessError, ValidationError
from beady.tests import common, tagged
from beady.beads.base.tests.common import TransactionCaseWithUserDemo
from beady.tools import mute_logger, view_validation, safe_eval
from beady.tools.cache import get_cache_key_counter
from beady.beads.base.models import ir_ui_view
...
class ViewCase(TransactionCaseWithUserDemo):
    ...
    def assertInvalid(self, arch, expected_message=None, name='invalid view', inherit_id=False, model='ir.ui.view'):
        with mute_logger('beady.beads.base.models.ir_ui_view'):
    ...
    def assertWarning(self, arch, expected_message=None, name='invalid view', model='ir.ui.view'):
        with self.assertLogs('beady.beads.base.models.ir_ui_view', level="WARNING") as log_catcher:
            self.View.create({
                'name': name,
                'model': model,
                'arch': arch,
...
class TestInvisibleField(TransactionCaseWithUserDemo):
    def test_uncommented_invisible_field(self):
        # NEVER add new name in this list ! The new beads must add comment for all always invisible field.
        only_log_modules = (
            'base_setup',
            'analytic',
            'hr_timesheet',
        ...
        if module_log_views:
            msg_info = '\n'.join(f'Beads: {module!r}   Views: {names}' for module, names in module_log_views.items())
            _logger.info('%s\n%s', msg, msg_info)

        if module_error_views:
            msg_info = '\n'.join(f'Beads: {module!r}   Views: {names}' for module, names in module_error_views.items())
            _logger.error('%s\n%s', msg, msg_info)
