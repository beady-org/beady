...
from beady.tools.mail import validate_url
from beady.tests.common import TransactionCase, BaseCase
...
class TestBeadsFileAccess(BaseCase):
    ...
    def test_file_path(self):
        ...
        # leading 'beads/' is ignored if present
        self.assertTrue(file_path("beads/web/__init__.py"))
        relpath = os.path.join('beads', relpath) # 'beads/base/tests/test_misc.py'
        self.assertEqual(__file__, file_path(relpath))

        # files in root_path are allowed
        self.assertTrue(file_path('tools/misc.py'))

        # errors when outside beads_paths
    ...
    def test_file_open(self):
        ...
        # leading 'beads/' is ignored if present
        self.assertCanRead("beads/web/__init__.py", "import")
        relpath = os.path.join('beads', relpath) # 'beads/base/tests/test_misc.py'
        self.assertCanRead(relpath, test_needle)
