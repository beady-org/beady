...
from beady.beads.base.tests import test_mail_examples
from beady.beads.base.tests.common import MockSmtplibCase
from beady.tests import tagged, users
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_priorities(self):
        ...
        simple Beady DB so we have to spoof the FROM otherwise we cannot send
        any email. """
    ...
    @mute_logger('beady.models.unlink', 'beady.beads.base.models.ir_mail_server')
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the beady-bin arguments. This config
        ...
        beady-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the beady-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from beady-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the beady-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the beady-bin argument
