...
@beady.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(beady.tests.TransactionCase):
...
class TestReportsRenderingCommon(beady.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        beady.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@beady.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@beady.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(beady.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
