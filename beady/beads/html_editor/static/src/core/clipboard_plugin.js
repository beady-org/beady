...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        const beadyHtml = dataHtmlElement.innerHTML;
        const beadyText = selection.textContent();
        ev.clipboardData.setData("text/plain", beadyText);
        ev.clipboardData.setData("text/html", beadyHtml);
        ev.clipboardData.setData("application/vnd.beady.beady-editor", beadyHtml);
        if (commonAncestor && commonAncestor.nodeType === Node.ELEMENT_NODE) {
            this.dispatchTo("normalize_handlers", commonAncestor);
        }
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteBeadyEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteBeadyEditorHtml(clipboardData) {
        const beadyEditorHtml = clipboardData.getData("application/vnd.beady.beady-editor");
        if (beadyEditorHtml) {
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.beady.beady-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.beady.beady-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Beady plugin?
/**
