...
import * as BeadyEditorLib from "@web_editor/js/editor/beady-editor/src/BeadyEditor";
...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/beady-editor/src/utils/utils';
...
const BeadyEditor = BeadyEditorLib.BeadyEditor;
const getDeepRange = BeadyEditorLib.getDeepRange;
const getInSelection = BeadyEditorLib.getInSelection;
const isProtected = BeadyEditorLib.isProtected;
const rgbToHex = BeadyEditorLib.rgbToHex;
const preserveCursor = BeadyEditorLib.preserveCursor;
const closestElement = BeadyEditorLib.closestElement;
const setSelection = BeadyEditorLib.setSelection;
const endPos = BeadyEditorLib.endPos;
const hasValidSelection = BeadyEditorLib.hasValidSelection;
const parseHTML = BeadyEditorLib.parseHTML;
const closestBlock = BeadyEditorLib.closestBlock;
const getRangePosition = BeadyEditorLib.getRangePosition;
const childNodeIndex = BeadyEditorLib.childNodeIndex;
const fillEmpty = BeadyEditorLib.fillEmpty;
const isVisible = BeadyEditorLib.isVisible;
const getDeepestPosition = BeadyEditorLib.getDeepestPosition;
const paragraphRelatedElements = BeadyEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.beadyEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.beadyEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.beadyEditor.editable)) {
                    this.beadyEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.beadyEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.beadyEditor._computeHistorySelection();
                } finally {
                    this.beadyEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.beadyEditor.withoutRollback(() => this.beadyEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.beadyEditor = new BeadyEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.beadyEditor.addEventListener('contentChanged', function () {
        ...
        if (this.beadyEditor.document !== document) {
            this.beadyEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.beadyEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.beadyEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.beadyEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.beadyEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeBeadyFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.beadyEditor.observerUnactive();
                const $field = $(this);
                ...
                self.beadyEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.beadyEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.beadyEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.beadyEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.beadyEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.beadyEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.beadyEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.beadyEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.beadyEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.beadyEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.beadyEditor.isMobile) {
                this.beadyEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.beadyEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.beadyEditor) {
            this.beadyEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.beadyEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.beadyEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.beadyEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.beadyEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.beadyEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper beady-editor">');
    ...
    getValue(options) {
        ...
        if (this.beadyEditor) {
            this.beadyEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.beadyEditor) : [];
     ...
     */
    setValue(value) {
        this.beadyEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.beadyEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.beadyEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.beadyEditor && !this.beadyEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.beadyEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.beadyEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.beadyEditor.editable) {
        if (this.beadyEditor) {
            this.beadyEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.beadyEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Beady field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeBeadyFieldChanges() {
        ...
        if (this.beadyFieldObservers) {
            for (let observerData of this.beadyFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const beadyFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $beadyFields = this.$editable.find(beadyFieldSelector);
            const renderingClassesSelector = this.beadyEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.beadyFieldObservers = [];

            $beadyFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.beadyEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $beadyFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseBeadyFieldObservers(field)` call.
                    this.__beadyFieldObserversToPause = this.beadyFieldObservers.filter(
                    ...
                    this._pauseBeadyFieldObservers();
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.beadyEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.beadyEditor.withoutRollback(() => {
                ...
                this.beadyFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseBeadyFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.beadyFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__beadyFieldObserversToPause || this.beadyFieldObservers;
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.beadyEditor.document.getSelection().getRangeAt(0).startContainer;
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.beadyEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.beadyEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.beadyEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.beadyEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.beadyEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.beadyEditor.editable,
                        link,
                ...
                this.beadyEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.beadyEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.beadyEditor.historySize() - 1;
            this.beadyEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.beadyEditor.historyUnpauseSteps();
                    this.beadyEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.beadyEditor.document);
        const params = {
            insert: content => {
                this.beadyEditor.historyPauseSteps();
                const insertedNodes = this.beadyEditor.execCommand('insert', content);
                this.beadyEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.beadyEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.beadyEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.beadyEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.beadyEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.beadyEditor.execCommand('unlink');
        }
    }
    /**
    ...
    showImageFullscreen(url) {
        ...
        this.beadyEditor.document.getSelection()?.collapseToEnd();
        this.beadyEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.beadyEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.beadyEditor.document);

        const editable = BeadyEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.beadyEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.beadyEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.beadyEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.beadyEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.beadyEditor.historyStep();
                this.beadyEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.beadyEditor.observerActive("unremovable");
            } else {
                this.beadyEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.beadyEditor.editable.focus();
        } else {
            const result = this.beadyEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.beadyEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.beadyEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await beady.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.beadyEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.beadyEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.beadyEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.beadyEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.beadyEditor.document).on('mouseup', mouseup);
            ...
            $(this.beadyEditor.document).on('mousedown', mousedown);
            ...
            this.beadyEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.beadyEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.beadyEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.beadyEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.beadyEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.beadyEditor.document.contains(element));
        ...
        if (coloredTds.length) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.beadyEditor.editable, { range });
            if (isSelForward) {
        ...
        this.beadyEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.beadyEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.beadyEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.beadyEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.beadyEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.beadyEditor.document.getSelection();
            const range = this.beadyEditor.document.createRange();
        ...
        this.beadyEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent BeadyEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.beadyEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.beadyEditor.document, `
                ...
                `).childNodes[0];
                this.beadyEditor.execCommand('insert', bannerElement);
                this.beadyEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), '❌', 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                isDisabled: () => !this.beadyEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.beadyEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.beadyEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.beadyEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.beadyEditor.execCommand('insert', parseHTML(this.beadyEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.beadyEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.beadyEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.beadyEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.beadyEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.beadyEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.beadyEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.beadyEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.beadyEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.beadyEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from beady field value
        // ZeroWidthSpace may be present from BeadyEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.beadyEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.beadyEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.beadyEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.beadyEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.beadyEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.beadyEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.beadyEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.beadyEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.beadyEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.beadyEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.beadyEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.beadyEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.beadyEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.beadyEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.beadyEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.beadyEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.beadyEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the beady bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.beadyEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.beadyEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.beadyEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.beadyEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.beadyEditor.historyResetFromSteps(steps, historyIds);
        this.beadyEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.beadyEditor && !this.beadyEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.beadyEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.beadyEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of BeadyEditorLib.getTraversedNodes(this.$editable[0])) {
