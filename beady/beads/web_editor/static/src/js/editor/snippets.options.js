...
const resetOuids = BeadyEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = publicWidget.Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.beadyEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.beadyEditor.automaticStepActive('preview_option');
        }
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.beadyEditor.unbreakableStepUnactive();
        }
        ...
        this.trigger_up('snippet_edition_request', {exec: async () => {
            ...
            if (shouldRecordUndo) {
                this.options.wysiwyg.beadyEditor.historyStep();
            }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.beadyEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                gridUtils._setElementToMaxZindex(backgroundGridEl, rowEl);
                self.options.wysiwyg.beadyEditor.observerActive('displayBackgroundGrid');
            }
            ...
            self.options.wysiwyg.beadyEditor.automaticStepUnactive('resizing');
            ...
            const iframeWindowMouseUp = function (ev) {
                ...
                self.options.wysiwyg.beadyEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.beadyEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async addElement(previewMode, widgetValue, params) {
        ...
        if (elementType === 'image') {
            ...
            if (!isImageSaved) {
                ...
                // media dialog closed.
                this.options.wysiwyg.beadyEditor.historyRevertCurrentStep();
                return;
            }
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.beadyEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.beadyEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.beadyEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.beadyEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.beadyEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.beadyEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.beadyEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.beadyEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.beadyEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.beadyEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    _clearListeners() {
        this.options.wysiwyg.beadyEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.beadyEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
    /**
     * @private
     */
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async removeStretch(previewMode, widgetValue, params) {
        this.options.wysiwyg.beadyEditor.historyPauseSteps();
        ...
        this.options.wysiwyg.beadyEditor.historyUnpauseSteps();
    },
    /**
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            clonedImgEl = img.cloneNode(true);
            this.options.wysiwyg.beadyEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.beadyEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.beadyEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.beadyEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Beady).
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        this.options.wysiwyg.beadyEditor.editable.focus();
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.beadyEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.beadyEditor.observerActive('_markColorLevel');
    },
});
