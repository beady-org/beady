...
import { closestElement, isUnremovable } from "@web_editor/js/editor/beady-editor/src/utils/utils";
...
import { Tooltip as BeadyTooltip } from "@web/core/tooltip/tooltip";
...
var SnippetEditor = publicWidget.Widget.extend({
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.beadyEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.beadyEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.beadyEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.beadyEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.beadyEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.beadyEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.beadyEditor.automaticStepActive();
        this.options.wysiwyg.beadyEditor.automaticStepSkipStack();
        this.options.wysiwyg.beadyEditor.unbreakableStepUnactive();
...
class SnippetsMenu extends Component {
    ...
    setup() {
        ...
        // Beady Editor uses the HTML Element to bind commands.
        ...
        onMounted(async () => {
            ...
            this.options.wysiwyg.beadyEditor.bindExecCommand(titleButtons);

            // Get table container and bind commands to Beady Editor.
            const customizeTableBlock = this.customizePanel.querySelector('#o-we-editor-table-container');
            this.options.wysiwyg.beadyEditor.bindExecCommand(customizeTableBlock);
    ...
    async start() {
        ...
        $(this.options.wysiwyg.beadyEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.beadyEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.beadyEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            const updateHistoryButtons = () => {
                this.state.canRedo = this.options.wysiwyg.beadyEditor.historyCanRedo();
                this.state.canUndo = this.options.wysiwyg.beadyEditor.historyCanUndo();
            };
            this.options.wysiwyg.beadyEditor.addEventListener('historyStep', updateHistoryButtons);
            this.options.wysiwyg.beadyEditor.addEventListener('observerApply', () => {
                $(this.options.wysiwyg.beadyEditor.editable).trigger('content_changed');
            });
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.beadyEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.beadyEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_ongoing_insertion');
                this.options.wysiwyg.beadyEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.beadyEditor.observerActive('dragAndDropCreateSnippet');
    ...
    _onClick(ev) {
        ...
        if ($oeStructure.length && !$oeStructure.children().length && this.snippets.size > 0) {
            ...
            this._activateSnippet(false).then(() => {
                this.$el.find('.oe_snippet').beadyBounce();
            });
            return;
        }
        this._activateSnippet($target);
   ...
   _onSnippetDragAndDropStart() {
    ...
    _showSnippetTooltip(snippetEl, delay = 1500) {
        ...
        this.hideShownTooltip = this.popover.add(snippetEl, BeadyTooltip, {
    ...
    _onSaveRequest(ev) {
        ...
        // If it's an BeadyEvent sent by sub-widgets, we prevent the event
        ...
        if (!ev.currentTarget.matches(".o_disabled") && ev.currentTarget.dataset.snippetGroup) {
            ...
            for (const el of $els) {
                if (!el.children.length) {
                    $(el).beadyBounce('o_we_snippet_area_animation');
                }
            }
        }
    }
    ...
    _checkEditorToolbarVisibility(e) {
        ...
        const selection = this.options.wysiwyg.beadyEditor.document.getSelection();
    ...
    _onRequestEditable(ev) {
        ev.data.callback($(this.options.wysiwyg.beadyEditor.editable));
    }
    /**
    ...
    _installModule(moduleID, snippetName) {
        ...
        const linkUrl = '/beady/action-base.open_module_tree/' + encodeURIComponent(moduleID);
