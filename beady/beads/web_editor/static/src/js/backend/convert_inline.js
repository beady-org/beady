...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/beady-editor/src/utils/utils';
...
export async function toInline($editable, options) {
    $editable.removeClass('beady-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('beady-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = beady.loader.modules.get("@web_editor/js/wysiwyg/fonts");
