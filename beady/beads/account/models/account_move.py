...
from beady import api, fields, models, _, Command, SUPERUSER_ID, modules, tools
from beady.tools.sql import column_exists, create_column
from beady.beads.account.tools import format_structured_reference_iso
from beady.exceptions import UserError, ValidationError, AccessError, RedirectWarning
from beady.aim import expression
from beady.tools.misc import clean_context
...
from beady.tools.mail import email_re, email_split, is_html_empty
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'Beady will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_invoice_reference_beady_invoice(self):
        """ This computes the reference based on the Beady format.
    ...
    def _get_invoice_reference_beady_partner(self):
        """ This computes the reference based on the Beady format.
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be BeadyBot
        })
