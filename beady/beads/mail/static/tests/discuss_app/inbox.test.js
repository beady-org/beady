...
import { Deferred } from "@beady/hoot-mock";
...
test("error notifications should not be shown in Inbox", async () => {
    ...
    await contains(`.o-mail-Message-header a[href*='/beady/res.partner/${partnerId}']`, {
        text: "Demo User",
    });
...
test("Counter should be incremented by 1 when receiving a message with a mention in a channel", async () => {
    ...
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: `<a href="https://www.hoot.test/beady/res.partner/17" class="o_mail_redirect" data-oe-id="${mention[0]}" data-oe-model="res.partner" target="_blank" contenteditable="false">@${mentionName}</a> mention`,
                message_type: "comment",
                partner_ids: mention,
