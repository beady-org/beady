...
import { beforeEach, expect, test } from "@beady/hoot";
import {
...
test("media dialog: upload", async function () {
    ...
    const anchorNode = queryOne(".beady-editor-editable p");
...
test("mention a partner", async () => {
    ...
    const anchorNode = queryOne(`[name='body'] .beady-editor-editable p`);
    setSelection({ anchorNode, anchorOffset: 0 });
    ...
    await press("enter");
    expect("[name='body'] .beady-editor-editable").toHaveInnerHTML(`
    <p>
        <a target="_blank" data-oe-protected="true" contenteditable="false" href="https://www.hoot.test/beady/res.partner/17" class="o_mail_redirect" data-oe-id="17" data-oe-model="res.partner">
            @Mitchell Admin
        </a>
    </p>`);
});
