...
import { reactive } from "@beady/owl";
...
/**
 * @return {Promise<{ SfuClient: import("@mail/../lib/beady_sfu/beady_sfu").SfuClient, SFU_CLIENT_STATE: import("@mail/../lib/beady_sfu/beady_sfu").SFU_CLIENT_STATE }>}
 */
const loadSfuAssets = memoize(async () => await loadBundle("mail.assets_beady_sfu"));
...
class Network {
    ...
    p2p;
    /** @type {import("@mail/../lib/beady_sfu/beady_sfu").SfuClient} */
    sfu;
    /** @type {array[{ name: string, f: EventListener }]} */
    _listeners = [];
    /**
     * @param {import("@mail/discuss/call/common/peer_to_peer").PeerToPeer} p2p
     * @param {import("@mail/../lib/beady_sfu/beady_sfu").SfuClient} [sfu]
     */
    constructor(p2p, sfu) {
        this.p2p = p2p;
        this.sfu = sfu;
...
export class Rtc extends Record {
    ...
    async _loadSfu() {
        const load = async () => {
            await loadSfuAssets();
            const sfuModule = beady.loader.modules.get("@mail/../lib/beady_sfu/beady_sfu");
