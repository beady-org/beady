...
async function openDiscussChannel(channelId, action) {
    const discussURLRegexes = [
        new RegExp("/beady/discuss"),
        new RegExp(`/beady/\\d+/action-${action}`),
        new RegExp(`/beady/action-${action}`),
    ];
    let targetClient;
    ...
    if (!targetClient) {
        targetClient = await self.clients.openWindow(
            `/beady/action-${action}?active_id=discuss.channel_${channelId}`
        );
    }
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            clients.openWindow(`/beady/${modelPath}/${res_id}`);
        }
    }
});
