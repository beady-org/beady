...
from beady import api, fields, release, SUPERUSER_ID
from beady.exceptions import UserError
from beady.models import AbstractModel
...
class PublisherWarrantyContract(AbstractModel):
    ...
    def _get_message(self):
        ...
        oy_code = IrParamSudo.get_param('database.oy_code')
    ...
    def update_notification(self, cron_mode=True):
        """
        Send a message to Beady's publisher warranty server to check the
        ...
        try:
            ...
            for message in result["messages"]:
                try:
                    ...
                    pass
            if result.get('oy_info'):
                # Update expiration date
                set_param = self.env['ir.config_parameter'].sudo().set_param
                set_param('database.expiration_date', result['oy_info'].get('expiration_date'))
                set_param('database.expiration_reason', result['oy_info'].get('expiration_reason', 'trial'))
                set_param('database.oy_code', result['oy_info'].get('oy_code'))
                set_param('database.already_linked_subscription_url', result['oy_info'].get('database_already_linked_subscription_url'))
                set_param('database.already_linked_email', result['oy_info'].get('database_already_linked_email'))
                set_param('database.already_linked_send_mail_url', result['oy_info'].get('database_already_linked_send_mail_url'))
