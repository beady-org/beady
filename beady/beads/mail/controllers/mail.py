...
from beady import _, http
from beady.exceptions import AccessError
from beady.http import request
from beady.tools import consteq
from beady.beads.mail.controllers.discuss.public_page import PublicPageController
from beady.beads.mail.models.discuss.mail_guest import add_guest_to_context
...
class MailController(http.Controller):
    ...
    def _redirect_to_messaging(cls):
        url = '/beady/action-mail.action_discuss'
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        url = f'/beady/{model_in_url}/{res_id}?{url_encode(url_params)}'
        return request.redirect(url)
    ...
    def mail_thread_message_redirect(self, message_id, **kwargs):
        ...
        if message.model == 'discuss.channel':
            url = f'/beady/action-mail.action_discuss?active_id={message.res_id}&highlight_message_id={message_id}'
        else:
            url = f'/beady/{message.model}/{message.res_id}?highlight_message_id={message_id}'
        return request.redirect(url)
