...
import beady.tests
from beady.tests import new_test_user


from unittest.mock import patch

from beady import release
from beady.beads import __path__ as __beads_path__
from beady.exceptions import UserError
from beady.tools import mute_logger


@beady.tests.tagged('post_install', '-at_install')
class TestImportModule(beady.tests.TransactionCase):
    ...
    def test_import_zip(self):
        ...
        with self.assertLogs('beady.beads.base.models.ir_module') as log_catcher:
            self.import_zipfile(files)
            self.assertIn('INFO:beady.beads.base.models.ir_module:module foo: no translation for language fr_FR', log_catcher.output)
    ...
    def test_import_zip_invalid_manifest(self):
        ...
        with (
            mute_logger("beady.beads.base_import_module.models.ir_module"),
    ...
    def test_import_zip_ignore_unexpected_data_extension(self):
        ...
        with self.assertLogs('beady.beads.base_import_module.models.ir_module') as log_catcher:
            self.import_zipfile(files)
            self.assertIn("INFO:beady.beads.base_import_module.models.ir_module:module foo: skip unsupported file res.partner.xls", log_catcher.output)
            self.assertIn("INFO:beady.beads.base_import_module.models.ir_module:Successfully imported module 'foo'", log_catcher.output)
            self.assertFalse(self.env.ref('foo.foo', raise_if_not_found=False))
    ...
    def test_import_zip_extract_only_useful(self):
        ...
        def _import_module(self, *args, **kwargs):
            ...
            beads_path.extend(__beads_path__)
        ...
        self.assertFalse(
            set(beads_path).difference(__beads_path__),
            'No directory must be added in the beads path during import')
...
class TestImportModuleHttp(TestImportModule, beady.tests.HttpCase):
