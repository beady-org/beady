...
from beady.exceptions import AccessDenied, AccessError, UserError
from beady.http import request
from beady.modules.module import adapt_version, MANIFEST_NAMES
from beady.aim.expression import is_leaf
from beady.release import major_version
from beady.tools import convert_csv_import, convert_sql_import, convert_xml_import, exception_to_unicode
from beady.tools import file_open, file_open_temporary_directory, ormcache

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.beady.org"
...
class IrModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the Beady Studio app."))
    ...
    def _get_missing_dependencies(self, zip_data):
        ...
        if unavailable_modules:
            description = _(
                ...
                " be found in the beads-path:\n"
            )
            for module in unavailable_modules:
                description += "- " + module + "\n"
            description += _(
                "\nYou may need the OY version to install the data module. Please visit "
                "https://www.beady.org/pricing-plan for more information.\n"
                "If you need Website themes, it can be downloaded from https://github.com/beady/design-themes.\n"
            )
        else:
            description = _(
