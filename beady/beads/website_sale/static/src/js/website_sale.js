        ...
        if (salePage.dataset.ecomZoomAuto && !uiUtils.isSmall()) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomBeady = $image.data("zoomBeady");
                    if (zoomBeady) {
                        zoomBeady.hide();
                        $image.unbind();
                    }
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoombeady-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.beadyEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.beadyEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
