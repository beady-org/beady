...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('beady-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(beadyPartnerData) {
        // Create the markers and cluster them on the map
        if (beadyPartnerData) { /* beady_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < beady_partner_data.counter; i++) {
                markerPromises.push(set_marker(beady_partner_data.partners[i]));
            }
    ...
    initializeMarkers(beady_partner_data);
