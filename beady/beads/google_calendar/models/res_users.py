...
from beady import api, fields, models, Command
from beady.beads.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from beady.beads.google_calendar.models.google_sync import google_calendar_token
from beady.beads.google_account.models import google_service
from beady.loglevels import exception_to_unicode
from beady.tools import str2bool
...
class User(models.Model):
    ...
    def _check_pending_beady_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not results or (not results.get('events') and not self._check_pending_beady_records()):
            return False
        ...
        # We apply Google updates only if their write date is later than the write date in Beady.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Beady before applying any updates,
        # and use these dates instead of the current live dates.
        beady_events = self.env['calendar.event'].browse((events - recurrences).beady_ids(self.env))
        beady_recurrences = self.env['calendar.recurrence'].browse(recurrences.beady_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in beady_recurrences}
        events_write_dates = {e.id: e.write_date for e in beady_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2beady(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2beady(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # Beady -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_beady2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_beady2google(calendar_service)

        return bool(results) and (bool(events | synced_events) or bool(recurrences | synced_recurrences))

    def _sync_single_event(self, calendar_service: GoogleCalendarService, beady_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2beady(event, default_reminders=default_reminders)
        # Beady -> Google
        beady_event.with_context(send_updates=send_updates)._sync_beady2google(calendar_service)
        return bool(beady_event | synced_events)
