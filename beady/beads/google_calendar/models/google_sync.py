...
from beady import api, fields, models, _
from beady.modules.registry import Registry
from beady.tools import ormcache_context, email_normalize
from beady.aim import expression
from beady.sql_db import BaseCursor

from beady.beads.google_calendar.utils.google_event import GoogleEvent
from beady.beads.google_calendar.utils.google_calendar import GoogleCalendarService
from beady.beads.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Beady database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Beady.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Beady DB (_sync_google2beady).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_beady2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2beady(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Beady. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Beady
        :param write_dates: A dictionary mapping Beady record IDs to their write dates.
        :return: synchronized beady recurrences
        """
        ...
        beady_values = [
            dict(self._beady_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_beady = self.with_context(dont_notify=True)._create_from_google(new, beady_values)
        cancelled = existing.cancelled()
        cancelled_beady = self.browse(cancelled.beady_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Beady.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_beady += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_beady.exists()._cancel()
        synced_records = new_beady + cancelled_beady
        pending = existing - cancelled
        pending_beady = self.browse(pending.beady_ids(self.env)).exists()
        for gevent in pending:
            beady_record = self.browse(gevent.beady_id(self.env))
            if beady_record not in pending_beady:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and beady server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than Beady's write_date.
            beady_record_write_date = write_dates.get(beady_record.id, beady_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not beady_record_write_date or updated >= pytz.utc.localize(beady_record_write_date):
                vals = dict(self._beady_values(gevent, default_reminders), need_sync=False)
                beady_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= beady_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Beady to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Beady to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
    ...
    @api.model
    def _beady_values(self, google_event: GoogleEvent, default_reminders=()):
