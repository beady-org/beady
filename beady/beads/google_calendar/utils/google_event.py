...
class GoogleEvent(abc.Set):
    ...
    Inspired by Beady recordset, one instance can be a single Google event or a
    ...
    def beady_id(self, env):
        self.beady_ids(env)  # load ids
        return self._beady_id

    def _meta_beady_id(self, dbname):
        """Returns the Beady id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_beady_id' % dbname)
        if o_id:
            return int(o_id)

    def beady_ids(self, env):
        ids = tuple(e._beady_id for e in self if e._beady_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_beady_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_beady_ids_from_metadata(env, model)

        return tuple(e._beady_id for e in self)

    def _load_beady_ids_from_metadata(self, env, model):
        unsure_beady_ids = tuple(e._meta_beady_id(env.cr.dbname) for e in self)
        beady_events = model.browse(_id for _id in unsure_beady_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Beady id.
        # Therefore, we only consider Beady records without google id when trying
        # to match events.
        o_ids = beady_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            beady_id = e._meta_beady_id(env.cr.dbname)
            if beady_id in o_ids:
                e._events[e.id]['_beady_id'] = beady_id

    def _load_beady_ids_from_db(self, env, model):
        beady_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in beady_events}  # {google_id: beady_id}
        existing_google_ids = beady_events.mapped('google_id')
        for e in self:
            beady_id = mapping.get(e.id)
            if beady_id:
                e._events[e.id]['_beady_id'] = beady_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Beady.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Beady (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Beady) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Beady several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_beady_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.beady_ids(env)
        events.beady_ids(env)

        return self.filter(lambda e: e._beady_id)
    ...
    def get_beady_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.beady_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.beady_id(self.env)).base_event_id
