...
@tagged("beadybot")
class TestBeadybot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestBeadybot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.beadybot = cls.env.ref("base.partner_root")
        ...
        cls.beadybot_ping_body = f'<a href="http:/.beady.org/beady/res.partner/{cls.beadybot.id}" class="o_mail_redirect" data-oe-id="{cls.beadybot.id}" data-oe-model="res.partner" target="_blank">@BeadyBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('beady.beads.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_beadybot()
        beadybot = self.env.ref("base.partner_root")
        beadybot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', beadybot.id)])
        self.assertEqual(len(beadybot_in_fetch_listeners), 1, 'beadybot should appear only once in channel_fetch_listeners')

    @mute_logger('beady.beads.mail.models.mail_mail')
    def test_beadybot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.beadybot_ping_body, 'partner_ids': [self.beadybot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.beadybot,
                answer=False
            )
        # Beadybot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.beadybot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last beadybot message instead
            sender=self.beadybot,
            answer=("@BeadyBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.beadybot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.beadybot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('beady.beads.mail.models.mail_mail')
    def test_beadybot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @beadybot right now", 'partner_ids': []})
        self.assertNextMessage(
