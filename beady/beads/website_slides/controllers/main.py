...
from beady import fields, http, tools, _
from beady.beads.website.controllers.main import QueryURL
from beady.beads.website.models.ir_http import sitemap_qs2dom
from beady.beads.website_profile.controllers.main import WebsiteProfile
from beady.exceptions import AccessError, ValidationError, UserError, MissingError
from beady.http import request, Response
from beady.aim import expression
from beady.tools import consteq, email_split
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Beady 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Beady 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Beady 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Beady 15.0 to 18.0.
