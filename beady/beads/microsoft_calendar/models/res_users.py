...
import logging
import requests
from beady.beads.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from beady.beads.microsoft_account.models import microsoft_service
from beady.beads.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Beady
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2beady(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Beady -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_beady2microsoft()
        ...
        (events - synced_events)._sync_beady2microsoft()
        self.sudo().microsoft_last_sync_date = fields.datetime.now()
