...
from beady.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Beady event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Beady events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Beady events
        # -----------------------------------------------------------------------------------------

        self.expected_beady_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_beady_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_beady_recurrency_events_from_outlook = [
            {
    ...
    def sync_beady_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Beady. A trick for deactivating the checking
    ...
    def assert_beady_event(self, beady_event, expected_values):
        """
        Assert that an Beady event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        beady_event_values = beady_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(beady_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, beady_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_beady_recurrence(self, beady_recurrence, expected_values):
        """
        Assert that an Beady recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        beady_recurrence_values = beady_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, beady_recurrence_values.get(k), msg=f"'{k}' mismatch")
