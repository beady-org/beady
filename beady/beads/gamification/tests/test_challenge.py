...
from beady.beads.gamification.tests.common import TransactionCaseGamification
...
class TestGamificationCommon(TransactionCaseGamification):
    ...
    def setUp(self):
        ...
        self.robot = self.env['res.users'].with_context(no_reset_password=True).create({
            'name': 'R2D2',
            'login': 'r2d2.beady.org',
            'email': 'r2d2.beady.org',
...
class test_challenge(TestGamificationCommon):
    ...
    @mute_logger('beady.models.unlink', 'beady.beads.mail', 'beady.beads.auth_signup')
