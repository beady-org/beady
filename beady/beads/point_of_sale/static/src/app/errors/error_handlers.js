...
import { beadyExceptionTitleMap, ErrorDialog } from "@web/core/errors/error_dialogs";
...
export function handleRPCError(error, dialog) {
    const { data } = error;
    if (beadyExceptionTitleMap.has(error.exceptionName)) {
        const title = beadyExceptionTitleMap.get(error.exceptionName).toString();
    ...
    if (originalError instanceof ConnectionLostError) {
        if (!env.services.pos.data.network.warningTriggered) {
            env.services.dialog.add(AlertDialog, {
                ...
                body: _t(
                    "Until the connection is reestablished, Beady Point of Sale will operate with limited functionality."
                ),
