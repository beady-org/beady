...
export class PosStore extends Reactive {
    ...
    async recordSynchronisation(data) {
        if (beady.debug === "assets") {
            console.info("Incoming synchronisation", data);
            console.info("Login number", beady.login_number, data.login_number);
            console.info("Session Ids", beady.pos_session_id, data.session_id);
        }

        if (data.login_number === beady.login_number || data.session_id !== beady.pos_session_id) {
            return;
        }
    ...
    async processServerData() {
        ...
        this.models["pos.session"].getFirst().login_number = parseInt(beady.login_number);
    ...
    redirectToBackend() {
        window.location = "/beady/action-point_of_sale.action_client_pos_menu";
    }
