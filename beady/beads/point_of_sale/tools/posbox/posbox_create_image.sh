...
current_repo="$(git remote get-url $current_remote | sed 's/.*github.com[\/:]//' | sed 's/\/beady.git//')"
read -p "Enter repo [${current_repo}]: " REPO
REPO="https://github.com/${REPO:-$current_repo}/beady.git"
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/beady"
...
if [ ! -d $CLONE_DIR ]; then
    ...
    echo "beads/web
beads/hw_*
beads/point_of_sale/tools/posbox/configuration
beady/
beady-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/beady/
echo "${VERSION_IOTBOX}" > "${MOUNT_POINT}"/var/beady/iotbox_version
