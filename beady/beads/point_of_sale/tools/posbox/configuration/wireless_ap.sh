...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ "${ESSID}" ] && [ "${PASSWORD}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/beady/beads/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		service beady restart # As this file is executed on boot, this line is responsible for restarting beady service on reboot
	fi
# wired
else
	killall nginx
