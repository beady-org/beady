...
echo  "alias beady='sudo systemctl stop beady; /usr/bin/python3 /home/pi/beady/beady-bin --config /home/pi/beady.conf --load=hw_drivers,hw_escpos,hw_posbox_homepage,point_of_sale,web'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias beady_logs='less +F /var/log/beady/beady-server.log'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias write_mode='sudo mount -o remount,rw / && sudo mount -o remount,rw /root_bypass_ramdisks'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias beady_conf='cat /home/pi/beady.conf'" | tee -a ~/.bashrc /home/pi/.bashrc
...
echo  "alias beady_luxe='printf \" ______\n< Luxe >\n ------\n        \\   ^__^\n         \\  (oo)\\_______\n            (__)\\       )\\/\\ \n                ||----w |\n                ||     ||\n\"'" | tee -a ~/.bashrc /home/pi/.bashrc
echo  "alias beady_start='sudo systemctl start beady'" >> /home/pi/.bashrc
echo  "alias beady_stop='sudo systemctl stop beady'" >> /home/pi/.bashrc
echo  "alias beady_restart='sudo systemctl restart beady'" >> /home/pi/.bashrc
echo "
beady_help() {
  echo 'Welcome to Beady IoTBox tools'
  echo 'beady                Starts/Restarts Beady server manually (not through beady.service)'
  echo 'beady_logs           Displays Beady server logs in real time'
  echo 'beady_conf           Displays Beady configuration file content'
  ...
  echo 'beady_start          Starts Beady service'
  echo 'beady_stop           Stops Beady service'
  echo 'beady_restart        Restarts Beady service'
  echo 'beady_dev <branch>   Resets Beady on the specified branch from beady-dev repository'
}
...
beady_dev() {
  ...
  pwd=\$(pwd)
  cd /home/pi/beady
  git remote add dev https://github.com/beady-dev/beady.git
...
# copy the beady.conf file to the overwrite directory
mv -v "/home/pi/beady/beads/point_of_sale/tools/posbox/configuration/beady.conf" "/home/pi/"
chown pi:pi "/home/pi/beady.conf"

apt-get update

# At the first start it is necessary to configure a password
# This will be modified by a unique password on the first start of Beady
...
wget 'https://nightly.beady.org/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.beady.org/master/iotbox/eftapi.so' -P /usr/lib/

groupadd usbusers
usermod -a -G usbusers pi
usermod -a -G lp pi
usermod -a -G input lightdm
mkdir -v /var/log/beady
chown pi:pi /var/log/beady
chown pi:pi -R /home/pi/beady/
...
echo "* * * * * rm /var/run/beady/sessions/*" | crontab -
...
systemctl enable beady.service
...
echo "beads/hw_drivers/iot_devices/" > /home/pi/beady/.git/info/exclude
