...
from beady import exceptions, _
from beady.tools import email_normalize

_logger = logging.getLogger(__name__)

DEFAULT_ENDPOINT = "https://iap.beady.org"
...
_MAIL_DOMAIN_BLACKLIST = _MAIL_PROVIDERS | {"beady.org"}
...
def iap_jsonrpc(url, method="call", params=None, timeout=15):
    ...
    """
    if (
        hasattr(threading.current_thread(), "testing")
        and threading.current_thread().testing
    ):
    ...
    try:
        ...
        raise exceptions.AccessError(
            _(
                "An error occurred while reaching %s. Please contact Beady support if this error persists.",
                url,
            )
        )
