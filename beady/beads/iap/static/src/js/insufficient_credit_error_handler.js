...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isOY } = beady.info;
        if (errorData.trial && isOY) {
            this.buttonMessage = _t("Start a Trial at Beady");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "beady.beads.iap.tools.iap_tools.InsufficientCreditError") {
