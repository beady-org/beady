...
from beady.tests import JsonRpcException, tagged
from beady.tools import mute_logger

from beady.beads.account_payment.tests.common import AccountPaymentCommon
from beady.beads.payment.tests.http_common import PaymentHttpCommon
from beady.beads.portal.controllers.portal import CustomerPortal
from beady.beads.sale.controllers.portal import PaymentPortal
from beady.beads.sale.tests.common import SaleCommon
...
class TestSalePayment(AccountPaymentCommon, SaleCommon, PaymentHttpCommon):
    ...
    def test_11_so_payment_link(self):
        ...
        with patch(
            'beady.beads.payment.controllers.portal.PaymentPortal'
        ...
        with mute_logger('beady.beads.payment.models.payment_transaction'):
    ...
    def test_12_so_partial_payment_link(self):
        ...
        with mute_logger('beady.beads.sale.models.payment_transaction'):
            tx_sudo._post_process()
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('beady.beads.sale.models.payment_transaction'), patch(
            'beady.beads.sale.models.sale_order.SaleOrder._create_invoices',
    ...
    def test_check_payment_access_token_before_rerouting_flow(self):
        ...
        ), patch('beady.beads.payment.utils.check_access_token') as check_payment_access_token_mock:
            try:
                payment_portal_controller._get_extra_payment_form_values(
                    sale_order_id=self.sale_order.id, access_token='whatever'
                )
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        ...
        with self.assertRaises(JsonRpcException, msg='beady.exceptions.ValidationError'):
    ...
    def test_partial_payment_confirm_order(self):
        ...
        with patch(
            'beady.beads.sale.models.sale_order.SaleOrder._send_order_notification_mail',
