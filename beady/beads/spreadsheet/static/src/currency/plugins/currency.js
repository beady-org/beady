...
import { EvaluationError, helpers, registries } from "@beady/o-spreadsheet";
...
export class CurrencyPlugin extends BeadyUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.beadyDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'BeadyDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("beadyCurrency", CurrencyPlugin);
