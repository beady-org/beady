...
import { parse, helpers, iterateAstNodes } from "@beady/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { loadBundle } from "@web/core/assets";
import { BeadySpreadsheetModel } from "@spreadsheet/model";
import { BeadyDataProvider } from "@spreadsheet/data_sources/beady_data_provider";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/beady_menu_link_cell";
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const beadyDataProvider = new BeadyDataProvider(env);
    const model = new BeadySpreadsheetModel(data, { custom: { beadyDataProvider } }, revisions);
    return model;
}

/**
 * @param {BeadySpreadsheetModel} model
 */
export async function waitForBeadySources(model) {
...
export async function waitForDataLoaded(model) {
    await waitForBeadySources(model);
    const beadyDataProvider = model.config.custom.beadyDataProvider;
    if (!beadyDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                beadyDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        beadyDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToBeady(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeBeadyData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, cell] of Object.entries(sheet.cells)) {
            ...
            if (containsLinkToBeady(evaluatedCell.link)) {
        ...
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("beady_")) {
                ...
                const img = beadyChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
 ...
 * @returns {boolean}
 */
function containsBeadyFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
 ...
 * @returns {string}
 */
function beadyChartToImage(model, figure) {
