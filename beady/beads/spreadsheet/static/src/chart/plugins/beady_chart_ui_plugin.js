...
export class BeadyChartUIPlugin extends BeadyUIPlugin {
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getBeadyChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getBeadyChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getBeadyChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getBeadyChartDataSourceId(chartId);
        return this.charts[dataSourceId];
    }

    /**
     * Get the callback used for onClick and onHover in an Beady Chart
     */
     ...
     * @return {Promise[]}
     */
    _getBeadyChartsWaitForReady() {
        return this.getters
            .getBeadyChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getBeadyChartDataSourceId(chartId) {
    ...
    _refreshBeadyCharts() {
        for (const chartId of this.getters.getBeadyChartIds()) {
            this._refreshBeadyChart(chartId);
        }
    }
}
