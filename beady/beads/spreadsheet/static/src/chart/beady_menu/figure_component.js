...
patch(spreadsheet.components.FigureComponent.prototype, {
    ...
    async navigateToBeadyMenu() {
        const menu = this.env.model.getters.getChartBeadyMenu(this.props.figure.id);
        if (!menu) {
    ...
    get hasBeadyMenu() {
        return this.env.model.getters.getChartBeadyMenu(this.props.figure.id) !== undefined;
    },
    async onClick() {
        if (this.env.isDashboard() && this.hasBeadyMenu) {
            this.navigateToBeadyMenu();
        }
    },
});
