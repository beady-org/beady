...
import { EvaluationError, CellErrorType } from "@beady/o-spreadsheet";
...
export class BeadyPivotLoader {
    /**
     * @param {BeadyDataProvider} beadyDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(beadyDataProvider, load) {
        /** @private @type {BeadyDataProvider} */
        this.beadyDataProvider = beadyDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.beadyDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.beadyDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<BeadyFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.beadyDataProvider.serverData, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.beadyDataProvider.serverData.fetch(
            "ir.model",
            "display_name_for",
            [[model]]
        );
