import { click } from "@beady/hoot-dom";
import { animationFrame } from "@beady/hoot-mock";
import { expect, test, beforeEach } from "@beady/hoot";
...
test("icon external link isn't on the chart when its not linked to an beady menu", async function () {
    ...
    await animationFrame();
    const beadyMenu = model.getters.getChartBeadyMenu(chartId);
    expect(beadyMenu).toBe(undefined, { message: "No menu linked with the chart" });

    const externalRefIcon = fixture.querySelector(".o-chart-external-link");
    expect(externalRefIcon).toBe(null);
});

test("icon external link is on the chart when its linked to an beady menu", async function () {
    const model = await createModelWithDataSource({
        serverData,
    });
    ...
    const chartMenu = model.getters.getChartBeadyMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Beady menu is linked to chart" });
    await animationFrame();
    expect(".o-chart-external-link").toHaveCount(1);
});

test("icon external link is not on the chart when its linked to a wrong beady menu", async function () {
    const model = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
...
test("click on icon external link on chart redirect to the beady menu", async function () {
    ...
    const chartMenu = model.getters.getChartBeadyMenu(chartId);
    expect(chartMenu.id).toBe(2, { message: "Beady menu is linked to chart" });
    await animationFrame();
...
test("Click on chart in dashboard mode redirect to the beady menu", async function () {
    const doActionStep = "doAction";
    mockActionService(doActionStep);
    const model = await createModelWithDataSource({
        serverData,
    ...
    await click(fixture.querySelector(".o-chart-container"));
    await animationFrame();
    // Clicking on a chart while on dashboard mode redirect to the beady menu
    expect.verifySteps([doActionStep]);
});
