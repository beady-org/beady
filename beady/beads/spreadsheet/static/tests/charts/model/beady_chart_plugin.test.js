...
import { BeadyBarChart } from "@spreadsheet/chart/beady_chart/beady_bar_chart";
import { BeadyChart } from "@spreadsheet/chart/beady_chart/beady_chart";
import { BeadyLineChart } from "@spreadsheet/chart/beady_chart/beady_line_chart";
...
test("Can add an Beady Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof BeadyBarChart).toBe(true);
...
test("Can add an Beady Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof BeadyLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Beady Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof BeadyChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Beady bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "beady_bar",
...
test("Beady pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "beady_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Beady line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "beady_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Beady chart", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "beady_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("beady_line");
...
test("Can undo/redo an Beady chart creation", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "beady_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_pie" });
    insertChartInSpreadsheet(model, "beady_bar");
    insertChartInSpreadsheet(model, "beady_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Beady chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof BeadyChart).toBe(true);
...
test("Can cut/paste Beady chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof BeadyChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Beady chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load beady chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert beady chart from a different model", async () => {
    const model = await createModelWithDataSource();
...
test("Beady chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove beady chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getBeadyChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getBeadyChartIds().length).toBe(0);
});

test("Beady chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getBeadyChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getBeadyChartDisplayName(chartId)).toBe("(#1) Beady Line Chart");
});
...
test("Displays correct thousand separator for positive value in Beady Bar chart Y-axis", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const runtime = model.getters.getChartRuntime(chartId);
...
test("Thousand separator in Beady Bar chart Y-axis is locale-dependent", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "beady_bar" });
