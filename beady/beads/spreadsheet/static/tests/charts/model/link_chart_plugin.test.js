...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartBeadyMenusReferences[chartId]).toBe(1, {
        message: "Link to beady menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartBeadyMenu(chartId);
...
test("Can undo-redo a LINK_BEADY_MENU_TO_CHART", async function () {
    ...
    expect(model.getters.getChartBeadyMenu(chartId).id).toBe(1);
    model.dispatch("REQUEST_UNDO");
    expect(model.getters.getChartBeadyMenu(chartId)).toBe(undefined);
...
test("Links of Beady charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartBeadyMenu(newChartId)).toEqual(
        model.getters.getChartBeadyMenu(chartId)
    );
});
