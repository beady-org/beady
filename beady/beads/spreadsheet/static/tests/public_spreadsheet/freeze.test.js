...
import { registries } from "@beady/o-spreadsheet";
...
import { freezeBeadyData } from "@spreadsheet/helpers/model";
import { BeadyPivot, BeadyPivotRuntimeDefinition } from "@spreadsheet/pivot/beady_pivot";
...
test("beady pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeBeadyData(model);
...
test("beady pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeBeadyData(model);
...
test("beady charts are replaced with an image", async function () {
...
test("beady links are replaced with their label", async function () {
    const view = {
        name: "an beady view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: { content: "[menu_xml](beady://ir_menu_xml_id/test_menu)" },
                    A2: { content: "[menu_id](beady://ir_menu_id/12)" },
                    A3: { content: `[beady_view](beady://view/${JSON.stringify(view)})` },
                    A4: { content: "[external_link](https:/.beady.org)" },
    ...
    const frozenData = await freezeBeadyData(model);
    expect(frozenData.sheets[0].cells.A1.content).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2.content).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3.content).toBe("beady_view");
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https:/.beady.org)");
