...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Beady](https:/.beady.org)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https:/.beady.org");
    openLink(cell.link, env);
    expect.verifySteps(["https:/.beady.org"]);
});
...
test("click a menu link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](beady://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
test("click a menu link [2]", async () => {
    ...
    setCellContent(model, "A1", `[a view](beady://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("an beady view");
    openLink(cell.link, env);
...
test("Click a link containing an action xml id", async () => {
    ...
    const model = new Model({}, { custom: { env } });
    setCellContent(model, "A1", `[an action link](beady://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
