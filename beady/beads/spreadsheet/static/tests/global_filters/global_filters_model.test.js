...
import { animationFrame, mockDate, mockTimeZone } from "@beady/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@beady/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getBeadyChartIds()[0];
    fieldMatching = model.getters.getBeadyChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("field matching is removed when an Beady chart is deleted", async function () {
    ...
    expect(model.getters.getBeadyChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getBeadyChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating a non-beady pivot should not crash on global filter", async () => {
    const grid = {
