 ...
 * @typedef {import("@spreadsheet/helpers/model").BeadySpreadsheetModel} BeadySpreadsheetModel
...
export function setupDataSourceEvaluation(model) {
    model.config.custom.beadyDataProvider.addEventListener("data-source-updated", () => {
 ...
 * @returns {Promise<BeadySpreadsheetModel>}
 */
export async function createModelWithDataSource(params = {}) {
    ...
    const model = new Model(params.spreadsheetData, {
        ...
        custom: {
            env,
            beadyDataProvider: new BeadyDataProvider(env),
            ...config?.custom,
        },
    });
