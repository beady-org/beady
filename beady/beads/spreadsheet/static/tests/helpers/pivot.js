...
import { BeadyPivot } from "@spreadsheet/pivot/beady_pivot";
...
export async function insertPivotInSpreadsheet(model, pivotId, params) {
    ...
    if (!(ds instanceof BeadyPivot)) {
        throw new Error("The pivot data source is not an BeadyPivot");
    }
    await ds.load();
 ...
 * @returns {Promise<{ model: BeadySpreadsheetModel, env: object, pivotId: string}>}
 */
