...
xml_id_url_prefix = "beady://ir_menu_xml_id/"

beady_view_link_prefix = "beady://view/"


def beady_charts(data):
    """return all beady chart definition in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet.get("figures", [])
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("beady_")
        ]
    return figures
...
def beady_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like beady://view/{... view data...}
    """
    return [
        json.loads(url[len(beady_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(beady_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = beady_charts(data)
    beady_version = data.get("beadyVersion", 1)
    if beady_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, beady_charts(data)).items(),
        extract_fields(beady_view_fields, beady_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartBeadyMenusReferences", {}).values()) | {
