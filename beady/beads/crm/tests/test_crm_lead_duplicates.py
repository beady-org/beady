...
from beady.beads.crm.tests.common import TestCrmCommon
from beady.beads.iap.tools import iap_tools
from beady.tests.common import tagged, users
...
class TestCRMLead(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.emails_provider_generic = {
            ...
            ('fp.beady.org', 'fp.beady.org'),
            ('fp.alias@mail.beady.org', 'fp.alias@mail.beady.org'),
        }
        cls.emails_provider_company = {
            ('robert.poilvert@mycompany.com', 'mycompany.com'),
            ('fp@subdomain.beady.org', 'subdomain.beady.org'),
        }
        ...
        cls.lead_generic_email_dupes = cls.env['crm.lead'].create([
            ...
            {
                'email_from': 'FP.beady.org',
                'name': 'Dupe2 of fp.beady.org (same email)',
                'type': 'lead',
            },
            # phone_sanitized based
            {
                'email_from': 'not.fp@not.beady.org',
                'name': 'Dupe3 of fp.beady.org (same phone sanitized)',
                'phone': '+1 202 555 0123',
                'type': 'lead',
            },
            {
                ...
                'name': 'Dupe4 of fp.beady.org (same phone sanitized)',
                'type': 'lead',
            },
            # same commercial entity
            {
                'name': 'Dupe5 of fp.beady.org (same commercial entity)',
                'partner_id': cls.test_partners[1].id,
            },
            {
                'name': 'Dupe6 of fp.beady.org (same commercial entity)',
    ...
    def test_assert_initial_values(self):
        ...
        self.assertEqual(lead_generic.email_domain_criterion, 'fp.beady.org')
        self.assertEqual(lead_generic.email_normalized, 'fp.beady.org')
