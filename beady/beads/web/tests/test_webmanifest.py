...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "Beady")
        self.assertEqual(data["scope"], "/beady")
        self.assertEqual(data["start_url"], "/beady")
        self.assertEqual(data["display"], "standalone")
        self.assertEqual(data["background_color"], "#00415a")
        self.assertEqual(data["theme_color"], "#00415a")
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/beady-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/beady-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/beady?menu_id="))
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/beady-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/beady")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/beady/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/beady-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/beady")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/beady-icon-ios.png"/>', document.text,
