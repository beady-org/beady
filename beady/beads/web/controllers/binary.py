...
try:
    ...
    from beady.tools._vendor.send_file import send_file

import beady
import beady.modules.registry
from beady import SUPERUSER_ID, _, http, api
from beady.beads.base.models.assetsbundle import ANY_UNIQUE
from beady.exceptions import AccessError, UserError
from beady.http import request, Response
from beady.tools import file_open, file_path, replace_exceptions, str2bool
from beady.tools.image import image_guess_size_from_field_name
from beady.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Beady is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Beady is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if beady.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=beady.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or beady.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Beady logo instead", exc_info=True)
