...
from beady import http, modules
from beady.exceptions import AccessError
from beady.http import request
from beady.tools import file_open, file_path, image_process
...
class WebManifest(http.Controller):
                ...
                shortcuts.append(
                    {
                        ...
                        "url": "/beady?menu_id=%s" % data.mapped("res_id")[0],
    ...
    def _get_webmanifest(self):
        web_app_name = (
            ...
            .sudo()
            .get_param("web.web_app_name", "Beady")
        )
        manifest = {
            "name": web_app_name,
            "scope": "/beady",
        ...
        manifest["icons"] = [
            {
                "src": "/web/static/img/beady-icon-%s.png" % size,
                "sizes": size,
                "type": "image/png",
            }
    ...
    def service_worker(self):
        response = request.make_response(
            ...
            [
                ...
                ("Service-Worker-Allowed", "/beady"),
            ],
        )
        return response

    def _get_service_worker_content(self):
        """Returns a ServiceWorker javascript file scoped for the backend (aka. '/beady')"""
        with file_open("web/static/src/service_worker.js") as f:
            body = f.read()
            return body

    def _icon_path(self):
        return "web/static/img/beady-icon-192x192.png"

    @http.route(
        "/beady/offline", type="http", auth="public", methods=["GET"], readonly=True
    )
    def offline(self):
        """Returns the offline page delivered by the service worker"""
        return request.render(
            "web.webclient_offline",
            {"beady_icon": base64.b64encode(file_open(self._icon_path(), "rb").read())},
        )
    ...
    def scoped_app(self, app_id, path="", app_name=""):
        ...
        scoped_app_values = {
            "app_id": app_id,
            "apple_touch_icon": "/web/static/img/beady-icon-ios.png",
            "app_name": app_name,
            "path": path,
    ...
    @http.route(
        "/web/manifest.scoped_app_manifest", type="http", auth="public", methods=["GET"]
    )
    def scoped_app_manifest(self, app_id, path, app_name=""):
        ...
        url are set to make sure no other installed PWA can overlap the scope (e.g. /beady)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
