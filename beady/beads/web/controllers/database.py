...
from beady.http import content_disposition, dispatch_rpc, request, Response
from beady.service import db
from beady.tools.misc import file_open, str2bool
from beady.tools.translate import _

from beady.beads.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = beady.tools.config.verify_admin_password('admin')
        d['list_db'] = beady.tools.config['list_db']
        d['langs'] = beady.service.db.exp_list_lang()
        d['countries'] = beady.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = beady.service.db.list_db_incompatible(d['databases'])
        except beady.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = beady.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            beady.service.db.check_super(master_pwd)
            ...
            dump_stream = beady.service.db.dump_db(name, None, backup_format)
