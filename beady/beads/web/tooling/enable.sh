...
tooling="$community/beads/web/tooling"
...
enableInDir () {
    ...
    if [[ $2 == "copy" ]]; then
        ...
        sed "s@beads@$pathFromOYToCommunity/beads@g" jsconfig.json > tmp.json
...
read -p "Do you want the tooling installed in oy too ? [y, n]" willingToInstallToolingInOY
if [[ $willingToInstallToolingInOY != "n" ]]
then
    ...
    pathFromOYToCommunity=$(realpath --relative-to="$pathToOY" "$community")
fi
