import { expect, getFixture, test } from "@beady/hoot";
import {
    contains,
    defineModels,
    fieldInput,
...
import { queryAllTexts, queryFirst } from "@beady/hoot-dom";
...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe.beady.org" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe.beady.org");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe.beady.org");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new.beady.org");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new.beady.org");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe.beady.org" },
        { id: 2, email: "jane.doe.beady.org" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe.beady.org");
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new.beady.org");
});
