...
import { Component, onMounted, xml } from "@beady/owl";
import {
    contains,
    defineActions,
    defineMenus,
...
import { queryAllAttributes, queryAllTexts, queryFirst } from "@beady/hoot-dom";
...
describe(`new urls`, () => {
    ...
    test(`menu loading`, async () => {
        redirect("/beady?menu_id=2");
        logHistoryInteractions();

        await mountWebClient();
        expect(`.test_client_action`).toHaveText("ClientAction_Id 2");
        expect(`.o_menu_brand`).toHaveText("App2");
        expect(browser.location.href).toBe("http://example.com/beady/action-1002", {
            message: "url now points to the default action of the menu",
        });
        expect.verifySteps(["pushState http://example.com/beady/action-1002"]);
    });

    test(`action and menu loading`, async () => {
        redirect("/beady/action-1001?menu_id=2");
        logHistoryInteractions();
        ...
        expect.verifySteps(["pushState http://example.com/beady/action-1001"]);
    });
    ...
    test(`initial loading with action tag`, async () => {
        redirect("/beady/__test__client__action__");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        await mountWithCleanup(WebClient, { env });
        expect(browser.location.href).toBe("http://example.com/beady/__test__client__action__", {
            message: "url did not change",
        });
        expect.verifySteps([]);
    });
    ...
    test(`fallback on home action if no action found`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady");
    ...
    test(`correctly sends additional context`, async () => {
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/beady/4/action-1001", {
            message: "url did not change",
        });
        expect.verifySteps([
            "/web/action/load",
    ...
    test(`supports action as xmlId`, async () => {
        redirect("/beady/action-wowl.client_action");
        logHistoryInteractions();
        ...
        expect(browser.location.href).toBe(
            ...
            "http://example.com/beady/action-1099",
            { message: "url did not change" }
        );
        expect.verifySteps(["pushState http://example.com/beady/action-1099"]);
    });
    ...
    test(`supports opening action in dialog`, async () => {
        ...
        expect(`.o_menu_brand`).toHaveCount(0);
        expect(browser.location.href).toBe("http://example.com/beady/action-wowl.client_action", {
    ...
    test(`should not crash on invalid state`, async () => {
        redirect("/beady/m-partner?view_type=list");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect.verifySteps(["/web/webclient/translations", "/web/webclient/load_menus"]);
        expect(browser.location.href).toBe("http://example.com/beady/m-partner?view_type=list", {
    ...
    test(`properly load client actions`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/HelloWorldTest", {
            message: "the url did not change",
        });
        expect.verifySteps([
    ...
    test(`properly load client actions with path`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/my-action");
        expect.verifySteps([
            "/web/webclient/translations",
            "/web/webclient/load_menus",
            "pushState http://example.com/beady/my-action",
        ]);
    });
    ...
    test(`properly load client actions with resId`, async () => {
        ...
        redirect("/beady/HelloWorldTest/12");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(browser.location.href).toBe("http://example.com/beady/HelloWorldTest/12", {
            message: "the url did not change",
        });
    ...
    test(`properly load client actions with updateActionState`, async () => {
        ...
        expect.verifySteps([
            "/web/webclient/translations",
            "/web/webclient/load_menus",
            "pushState http://example.com/beady/HelloWorldTest/12",
        ]);
    });
    ...
    test(`properly load client actions with resId and path (1)`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/my_client/12");
        // Breadcrumb should have only one item, the client action don't have a LazyController (a multi-record view)
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
            "Client Action DisplayName",
        ]);
        expect.verifySteps([
            ...
            "/web/webclient/load_menus",
            "resId:12",
            "pushState http://example.com/beady/my_client/12",
        ]);
    });
    ...
    test(`properly load client actions with LazyTranslatedString displayName`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/my_client");
        // Breadcrumb should have only one item, the client action don't have a LazyController (a multi-record view)
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
    ...
    test(`properly load act window actions`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/action-1", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`properly load records`, async () => {
        redirect("/beady/m-partner/2");
        logHistoryInteractions();
        stepAllNetworkCalls();

        await mountWebClient();
        expect(`.o_form_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/beady/m-partner/2", {
            message: "the url did not change",
        });
    ...
    test(`properly load default record`, async () => {
        redirect("/beady/action-3/new");
        logHistoryInteractions();
        stepAllNetworkCalls();

        await mountWebClient();
        expect(`.o_form_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/beady/action-3/new", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`load requested view for act window actions`, async () => {
        redirect("/beady/action-3?view_type=kanban");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(`.o_kanban_view`).toHaveCount(1);
        expect(browser.location.href).toBe("http://example.com/beady/action-3?view_type=kanban", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
    ...
    test(`lazy load multi record view if mono record one is requested`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/action-3/2", {
            message: "the url did not change",
        });
        expect.verifySteps([
            "/web/webclient/translations",
        ...
        expect(browser.location.href).toBe("http://example.com/beady/action-3");
        expect.verifySteps(["pushState http://example.com/beady/action-3"]);
    });
    ...
    test(`go back with breadcrumbs after doAction`, async () => {
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/beady/action-4");
        expect.verifySteps(["pushState http://example.com/beady/action-4"]);
        expect(queryAllTexts`.breadcrumb-item, .o_breadcrumb .active`).toEqual([
            "Partners Action 4",
        ]);
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/beady/action-4/action-3/2");
        // pushState was called only once
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/beady/action-4/action-3/2",
        ]);
        ...
        expect.verifySteps([
            ...
            "pushState http://example.com/beady/action-4",
        ]);
    });
    ...
    test(`lazy loaded multi record view with failing mono record one`, async () => {
        ...
        await animationFrame(); // pushState is debounced
        expect(browser.location.href).toBe("http://example.com/beady/action-3/action-1");
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/beady/action-3/action-1",
        ]);
    ...
    test(`should push the correct state at the right time`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/action-3/1");
        // should push the state if it changes afterwards
        expect.verifySteps([
            "Update the state without updating URL, nextState: actionStack,action,globalState",
            "pushState http://example.com/beady/action-3/1",
        ]);
    });
    ...
    test(`load state supports being given menu_id alone`, async () => {
        ...
        expect.verifySteps([
            ...
            "get_views",
            "web_search_read",
            "pushState http://example.com/beady/action-1",
        ]);
    });
    ...
    test(`load state: in a form view, no id in initial state`, async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/action-999/new");
    });
    ...
    test(`load state: in a form view, wrong id in the state`, async () => {
        ...
        redirect("/beady/action-1000/999");
        logHistoryInteractions();

        await mountWebClient();
        expect(`.o_list_view`).toHaveCount(1);
        expect(`.o_notification_body`).toHaveCount(1, { message: "should have a notification" });
        expect(browser.location.href).toBe("http://example.com/beady/action-1000", {
            message: "url reflects that we are not on the record",
        });
        expect.verifySteps(["pushState http://example.com/beady/action-1000"]);
        expect.verifyErrors([
    ...
    test(`server action loading with id`, async () => {
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/beady/action-2/2", {
            message: "url did not change",
        });
    ...
    test("server action returning act_window", async () => {
        ...
        redirect("/beady/my-path/2");
        logHistoryInteractions();
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/beady/my-path/2", {
            message: "url did not change",
        });
        expect(router.current).toEqual({
            action: "my-path",
    ...
    test(`state with integer active_ids should not crash`, async () => {
        redirect("/beady/action-2?active_ids=3");
        logHistoryInteractions();
        ...
        await mountWebClient();
        expect(browser.location.href).toBe("http://example.com/beady/action-2?active_ids=3", {
            message: "url did not change",
        });
    ...
    test(`initial action crashes`, async () => {
        ...
        redirect("/beady/__test__client__action__?menu_id=1");
        logHistoryInteractions();

        await mountWebClient();
        expect.verifySteps(["clientAction setup"]);
        expect(browser.location.href).toBe(
            "http://example.com/beady/__test__client__action__?menu_id=1",
            {
                message: "url did not change",
            }
        );
    ...
    test("all actions crashes", async () => {
        expect.errors(2);
        redirect("/beady/m-partner/2/m-partner/1");
        logHistoryInteractions();
        stepAllNetworkCalls();
        onRpc("web_read", () => Promise.reject());
    ...
    test(`initial loading with multiple path segments loads the breadcrumbs`, async () => {
        ...
        redirect("/beady/partners/2/action-28/1");
        logHistoryInteractions();
        stepAllNetworkCalls();
        ...
        expect(browser.location.href).toBe("http://example.com/beady/partners/2/action-28/1", {
            message: "url did not change",
        });
        expect.verifySteps([
            "/web/action/load_breadcrumbs",
    ...
    test(`don't load controllers when load action new`, async () => {
        ...
        Partner._views["form,false"] = /* xml */ `
            <form string="Partner">
                <sheet>
                    <a href="http://example.com/beady/action-5" class="clickMe">clickMe</a>
                    <group>
                        <field name="display_name"/>
                        <field name="foo"/>
                    </group>
        ...
        expect.verifySteps([
            "web_search_read",
            "has_group",
            "pushState http://example.com/beady/action-3",
        ]);
    });
    ...
    test("properly load previous action when error", async () => {
        ...
        expect.errors(1);
        redirect("/beady/action-3/1/m-partner");
        logHistoryInteractions();
        stepAllNetworkCalls();
        onRpc("web_read", () => Promise.reject());
    ...
    test("properly reload dynamic actions from sessionStorage", async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/beady/m-partner/1/m-partner");
