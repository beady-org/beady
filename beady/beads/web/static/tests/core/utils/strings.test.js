...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.beady.org">beady<a>`)).toBe(
        `&lt;a href=&quot;https://www.beady.org&quot;&gt;beady&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.beady.org'>beady<a>`)).toBe(
        `&lt;a href=&#x27;https://www.beady.org&#x27;&gt;beady&lt;a&gt;`
    );
    expect(escape("<a href='https://www.beady.org'>Beady`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.beady.org&#x27;&gt;Beady&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.beady]")).toBe("\\[wowl\\.beady\\]");
    expect(escapeRegExp("^beady.define([.]*)$")).toBe("\\^beady\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@beady")).toBe(false);
    expect(isEmail("test@beady.beady.org")).toBe(false);
    expect(isEmail("te st.beady.org")).toBe(false);

    expect(isEmail("test.beady.org")).toBe(true);
});
