...
import { describe, dryRun, globals, start, stop } from "@beady/hoot";
import { Deferred, microTick } from "@beady/hoot-dom";
import { watchKeys, watchListeners } from "@beady/hoot-mock";
import { whenReady } from "@beady/owl";
 ...
 *
 * @typedef {{
 *  beads?: Iterable<string>;
 * }} ModuleSetParams
 */

const { fetch: realFetch } = globals;
const { define, loader } = beady;
 ...
 * @param {Set<string>} additionalBeads
 */
const defineModuleSet = async (entryPoints, additionalBeads) => {
    /** @type {ModuleSet} */
    const moduleSet = {};
    if (additionalBeads.has("*")) {
        // Use all beads
        moduleSet.beadsKey = "*";
        ...
        for (const entryPoint of entryPoints) {
            additionalBeads.add(getAddonName(entryPoint));
        }
        const beads = await fetchDependencies(additionalBeads);
        ...
        const filter = (path) => beads.has(getAddonName(path));

        // Module names are cached for each configuration of beads
        const joinedBeads = [...beads].sort().join(",");
        if (!moduleNamesCache.has(joinedBeads)) {
        ...
        moduleSet.beadsKey = joinedBeads;
        moduleSet.filter = filter;
        moduleSet.moduleNames = moduleNamesCache.get(joinedBeads);
    }
...
/**
 * @param {Set<string>} beads
 */
const fetchDependencies = async (beads) => {
    // Fetch missing dependencies
    const beadsToFetch = [];
    for (const addon of beads) {
        if (!dependencyCache[addon] && !DEFAULT_ADDONS.includes(addon)) {
            beadsToFetch.push(addon);
            dependencyCache[addon] = new Deferred();
        }
    }
    if (beadsToFetch.length) {
        ...
        dependencyBatch.push(...beadsToFetch);
    ...
    await Promise.all([...beads].map((addon) => dependencyCache[addon]));

    return getDependencies(beads);
};
...
/**
 * @param {Iterable<string>} beads
 */
const getDependencies = (beads) => {
...
const runTests = async () => {
    ...
    for (const moduleName of testModuleNames) {
        ...
        if (currentBeadsKey !== moduleSet.beadsKey) {
            ...
            currentBeadsKey = moduleSet.beadsKey;
        }
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        beady.define = this.define.bind(this);
        beady.loader = this;
    }
...
const AUTO_INCLUDED_ADDONS = {
    /**
     * spreadsheet beads defines a module that does not starts with `@spreadsheet` but `@beady` (`@beady/o-spreadsheet)
     * To ensure that this module is loaded, we have to include `beady` in the dependencies
     */
    spreadsheet: ["beady"],
    /**
     * Add all view types by default
     */
    web_oy: ["web_gantt", "web_grid", "web_map"],
};
const CSRF_TOKEN = beady.csrf_token;
...
const R_DEFAULT_MODULE = /^@beady\/(owl|hoot)/;
