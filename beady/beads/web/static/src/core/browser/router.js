import { EventBus } from "@beady/owl";
...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "beady";
 ...
 * external website). Unfortunately, Beady wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/beady") &&
            (["/web", "/beady"].includes(url.pathname) || url.pathname.startsWith("/beady/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/beady") && url.hash) {
