...
function BeadyEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
BeadyEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
BeadyEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Beady where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new BeadyEvent(this, name, info);
