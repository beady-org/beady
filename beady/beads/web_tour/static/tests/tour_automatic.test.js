...
import { afterEach, beforeEach, describe, expect, test } from "@beady/hoot";
import { animationFrame, queryFirst } from "@beady/hoot-dom";
import { advanceTime, runAllTimers } from "@beady/hoot-mock";
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await beady.startTour("tour2", { mode: "auto" }); // Use beady to run tour from registry because this is a test tour
...
test("a failing tour with disabled element", async () => {
    ...
    await beady.startTour("tour3", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
...
test("a failing tour logs the step that failed", async () => {
    ...
    await beady.startTour("tour1", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("check tour with inactive steps", async () => {
    ...
    await beady.startTour("pipu_tour", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("automatic tour with invisible element", async () => {
    ...
    await beady.startTour("tour_de_wallonie", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await advanceTime(10000);
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await beady.startTour("tour_des_flandres", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await waitForStep();
...
test("check not possible to click below modal", async () => {
    ...
    await beady.startTour("tour_check_modal", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    await advanceTime(10000);
...
test("a tour where hoot trigger failed", async () => {
    ...
    await beady.startTour("tour_hoot_failed", { mode: "auto" });
    await waitForStep(true);
    await waitForStep();
    await waitForStep();
    expect.verifySteps([
...
test("check for undeterminisms", async () => {
    ...
    await beady.startTour("tour_und", {
        mode: "auto",
