...
import { queryAll, queryFirst, queryOne } from "@beady/hoot-dom";
import { Component, useState, useExternalListener } from "@beady/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasBeadyClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasBeadyClass;
