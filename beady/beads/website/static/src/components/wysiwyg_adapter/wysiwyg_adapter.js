...
import { isMediaElement } from '@web_editor/js/editor/beady-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@beady/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.beadyEditor.options.filterMutationRecords;
        Object.assign(this.beadyEditor.options, {
            /**
             * @override
             */
        ...
        // Disable BeadyEditor observer's while setting up classes
        this.beadyEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with beadyEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.beadyEditor.observerActive();
    }
    /**
    ...
    async destroy() {
        ...
        const mod = await beady.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await beady.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.beadyEditor.filterMutationRecords(records);
        ...
        this.beadyEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.beadyEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by BeadyEditor when text is entered.
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await beady.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/beady/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
