...
import * as BeadyEditorLib from "@web_editor/js/editor/beady-editor/src/utils/utils";
import { Component, onMounted, onWillStart, useEffect, useRef, useState } from "@beady/owl";
...
const getTraversedNodes = BeadyEditorLib.getTraversedNodes;
...
export class WebsiteSnippetsMenu extends weSnippetEditor.SnippetsMenu {
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Beady's D&D
        // There is currently no way in Beady D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.beadyEditor.document.getSelection();
    }
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.beadyEditor.computeFontSizeSelectorValues();
    }
    /**
    * @override
    */
    ...
    _handleTextOptions(classSelector, optionClassList, textOptionsPostActivate = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = BeadyEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.beadyEditor.historyResetLatestComputedSelection();
