...
class TestWebsiteAssets(beady.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/beady')
        check_asset()
        self.url_open(domain_2 + '/beady')
        check_asset()
        self.url_open(domain_1 + '/beady')
        check_asset()
        self.url_open(domain_2 + '/beady')
...
class TestWebAssets(beady.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with beady.tools.mute_logger('beady.beads.web.controllers.binary'):
            self.assertEqual(
