...
from beady.beads.web_editor.controllers.main import Web_Editor


@beady.tests.tagged('-at_install', 'post_install')
class TestUiCustomizeTheme(beady.tests.HttpCase):
...
class TestUiHtmlEditor(HttpCaseWithUserDemo):
    ...
    def test_code_editor_usable(self):
        ...
        url = '/beady/action-website.website_preview'
        self.start_tour(url, 'website_code_editor_usable', login='admin')


@beady.tests.tagged('external', '-standard', '-at_install', 'post_install')
...
@beady.tests.tagged('-at_install', 'post_install')
class TestUiTranslate(beady.tests.HttpCase):
...
class TestUi(beady.tests.HttpCase):

    def test_01_admin_tour_homepage(self):
        self.start_tour("/beady", 'homepage', login='admin')
    ...
    def test_10_website_conditional_visibility(self):
        ...
        self.start_tour('/beady', 'conditional_visibility_2', login='admin')
    ...
    def test_19_website_page_options(self):
        self.start_tour("/beady", "website_page_options", login="admin")
    ...
    def test_21_website_start_cloned_snippet(self):
        self.start_tour('/beady', 'website_start_cloned_snippet', login='admin')

    def test_22_website_gray_color_palette(self):
        self.start_tour('/beady', 'website_gray_color_palette', login='admin')
    ...
    def test_26_website_media_dialog_icons(self):
        self.env.ref('website.default_website').write({
            'social_twitter': 'https://twitter.com/Beady',
            'social_facebook': 'https://www.facebook.com/Beady',
            'social_linkedin': 'https://www.linkedin.com/company/beady',
            'social_youtube': 'https://www.youtube.com/user/Beadyonline',
            'social_github': 'https://github.com/beady',
            'social_instagram': 'https://www.instagram.com/explore/tags/beady/',
            'social_tiktok': 'https://www.tiktok.com/@beady',
        })
        self.start_tour("/", 'website_media_dialog_icons', login='admin')

    def test_27_website_clicks(self):
        self.start_tour('/beady', 'website_click_tour', login='admin')
