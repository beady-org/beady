...
from beady import http, models, fields, _
from beady.exceptions import AccessError, UserError
from beady.http import request, SessionExpiredException
from beady.aim import expression
from beady.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from beady.beads.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from beady.beads.base.models.ir_qweb import QWebException
from beady.beads.portal.controllers.portal import pager as portal_pager
from beady.beads.portal.controllers.web import Home
from beady.beads.web.controllers.binary import Binary
from beady.beads.web.controllers.session import Session
from beady.beads.website.tools import get_base_domain
from beady.tools.json import scriptsafe as json
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/beady?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': beady.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/beady/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/beady/ir.ui.view/{page.get('view_id')}")
