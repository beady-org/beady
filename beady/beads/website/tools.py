...
import beady.http
from beady.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    # TODO move MockRequest to a package in beads/web/tests
    from beady.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{beady.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
        ...
        type='http',
        future_response=beady.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            beady.http.get_default_session(),
            sale_order_id=sale_order_id,
            website_sale_current_pl=website_sale_current_pl,
            context={'lang': ''},
        ),
        geoip=beady.http.GeoIP('127.0.0.1'),
    ...
    if country_code:
        request.geoip._city_record = beady.http.geoip2.models.City({'country': {'iso_code': country_code}})
    ...
    #
    #   router = beady.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(beady.http._request_stack.pop)
        s.enter_context(patch('beady.http.root.get_db_router', router))
