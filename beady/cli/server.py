...
"""
Beady - Server
Beady is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - Beady SA
"""
...
__author__ = beady.release.author
__version__ = beady.release.version

# Also use the `beady` logger for the main script.
_logger = logging.getLogger('beady')

re._MAXCACHE = 4096  # default is 512, a little too small for beady
...
def report_configuration():
    ...
    """
    config = beady.tools.config
    _logger.info("Beady version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('beads paths: %s', beady.beads.__path__)
...
def setup_pid_file():
    ...
    """
    config = beady.tools.config
    if not beady.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = beady.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            beady.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = beady.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = beady.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    beady.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                beady.service.db._create_empty_database(db_name)
            ...
            except beady.service.db.DatabaseExists:
                pass
    ...
    setup_pid_file()
    rc = beady.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the beady server (default command)"""
    def run(self, args):
        beady.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
