...
class Db(Command):
    ...
    def run(self, cmdargs):
        ...
        parser.add_argument('--beads-path')
        ...
        load = subs.add_parser(
            ...
            description="Loads a dump file into beady, dump file can be a URL. "
        ...
        config.parse_config([
            ...
            if v is not None
            if k in ['config', 'data_dir', 'beads_path'] or k.startswith(('db_', 'pg_'))
            for val in [
                '--data-dir' if k == 'data_dir'\
                    else '--beads-path' if k == 'beads_path'\
                    else f'--{k}',
                v,
            ]
        ], setup_logging=True)
