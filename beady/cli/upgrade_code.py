...
* addon: the beady addon in which the file is;
...
try:
    ...
    from beady.modules import initialize_sys_path
    from beady.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via beady-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing beady
...
class FileManager:
    ...
    def __init__(self, beads_path: list[str], glob: str = '**/*') -> None:
        self.beads_path = beads_path
        self.glob = glob
        self._files = {
            str(path): FileAccessor(path, Path(addon_path))
            for addon_path in beads_path
...
def migrate(
    ...
    file_manager = FileManager(beads_path, glob)
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /beady/upgrade_code """
    name = 'upgrade_code'
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            prog=(
                f"{self.prog_name} [--beads-path=PATH,...] {self.name}"
                if initialize_sys_path else
                self.prog_name
            ),
            description=__doc__.replace('/beady/upgrade_code', str(UPGRADE)),
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            config['beads_path'] = options.beads_path
            initialize_sys_path()
            options.beads_path = beady.beads.__path__
        else:
            options.beads_path = [p for p in options.beads_path.split(',') if p]
        if not options.beads_path:
            self.parser.error("--beads-path is required when used standalone")
