...
import beady
import beady.modules.db
import beady.modules.graph
import beady.modules.migration
import beady.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_beady_module
...
def force_demo(env):
    ...
    """
    graph = beady.modules.graph.Graph()
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = beady.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = beady.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = beady.sql_db.sql_counter
        ...
        load_beady_module(package.name)

        if new_install:
            py_module = sys.modules['beady.beads.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = beady.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), beady.sql_db.sql_counter
                test_results = loader.run_suite(suite)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = beady.sql_db.sql_counter - tests_q0
        ...
        extra_queries = beady.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   beady.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not beady.modules.db.is_initialized(cr):
        ...
        if not graph:
            _logger.critical('module base cannot be loaded! (hint: verify beads-path)')
            raise ImportError('Module `base` cannot be loaded! (hint: verify beads-path)')

        if update_module and beady.tools.sql.table_exists(cr, 'ir_model_fields'):
        ...
        migrations = beady.modules.migration.MigrationManager(cr, graph)
        for package in graph:
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['beady.beads.%s' % (pkg.name,)]
                ...
                registry = beady.modules.registry.Registry.new(
