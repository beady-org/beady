...
import beady
from beady.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from beady.sql_db import TestCursor
from beady.tools import (
    config, lazy_classproperty,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from beady.tools.func import locked
from beady.tools.lru import LRU
from beady.tools.misc import Collector, format_frame
...
_schema = logging.getLogger('beady.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                beady.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                beady.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from beady.tests.result import BeadyTestResult  # noqa: PLC0415
            self._assertion_report = BeadyTestResult()
        else:
        ...
        self._db = beady.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = beady.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = beady.modules.db.has_unaccent(cr)
            self.has_trigram = beady.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = beady.api.Environment(cr, SUPERUSER_ID, context)
