...
import beady
import beady.tools as tools
import beady.release as release
...
MANIFEST_NAMES = ('__manifest__.py', '__beady__.py')
README = ['README.rst', 'README.md', 'README.txt']

_DEFAULT_MANIFEST = {
    #beads_path: f'/path/to/the/beads/path/of/{module}',  # automatic
    'application': False,
    'bootstrap': False,  # web
    'assets': {},
    'author': 'Beady S.A.',
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `beady.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^beady\.beads\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("beady.beads.base.maintenance.migrations", "beady.upgrade")
...
def initialize_sys_path():
    """
    Setup the beads path ``beady.beads.__path__`` with various defaults
    and explicit directories.
    """
    # hook beady.beads on data dir
    dd = os.path.normcase(tools.config.beads_data_dir)
    if os.access(dd, os.R_OK) and dd not in beady.beads.__path__:
        beady.beads.__path__.append(dd)

    # hook beady.beads on beads paths
    for ad in tools.config['beads_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in beady.beads.__path__:
            beady.beads.__path__.append(ad)

    # hook beady.beads on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'beads')))
    if base_path not in beady.beads.__path__ and os.path.isdir(base_path):
        beady.beads.__path__.append(base_path)

    # hook beady.upgrade on upgrade-path
    from beady import upgrade
    ...
    # create decrecated module alias from beady.beads.base.maintenance.migrations to beady.upgrade
    spec = importlib.machinery.ModuleSpec("beady.beads.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["beady.beads.base.maintenance"] = maintenance_pkg
    sys.modules["beady.beads.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from beady to beady and "crm"-like to beady.beads
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    Search the beads paths and return the first path where the given
    module is found. If downloaded is True, return the default beads
    path if nothing else is found.

    """
    if re.search(r"[\/\\]", module):
        return False
    for adp in beady.beads.__path__:
    ...
    if downloaded:
        return opj(tools.config.beads_data_dir, module)
    if display_warning:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(beady.beads.__path__, key=len, reverse=True)
...
def module_manifest(path):
    ...
    for manifest_name in MANIFEST_NAMES:
        ...
        if os.path.isfile(candidate):
            if manifest_name == '__beady__.py':
                warnings.warn(
                    "__beady__.py manifests are deprecated since 17.0, "
...
def load_manifest(module, mod_path=None):
    ...
    manifest['beads_path'] = normpath(opj(mod_path, os.pardir))
...
def load_beady_module(module_name):
    """ Load an Beady module, if not already loaded.
    ...
    qualname = f'beady.beads.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in beady.beads.__path__:
        if not os.path.exists(ad):
            _logger.warning("beads path does not exist: %s", ad)
            continue
