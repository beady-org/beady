...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://usp.sa',
            'reportgz': False,
            'root_path': None,
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.beadyrc (or to ~/.beady_serverrc if it exists)")
        ...
        group.add_option("--beads-path", dest="beads_path",
                         help="specify additional beads paths (separated by commas).",
                         action="callback", callback=self._check_beads_path, nargs=1, type="string")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Beady data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'beady_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "beady.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="beady.http:DEBUG", help='shortcut for --log-handler=beady.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="beady.sql_db:DEBUG", help='shortcut for --log-handler=beady.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Beady to another language. "
        ...
        group.add_option("--aim-memory-count-limit", dest="aim_memory_count_limit", my_default=0,
                         help="Force a limit on the maximum number of records kept in the virtual "
                              "aim_memory tables. By default there is no limit.",
                         type="int")
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes beady.tools.config and beady.conf (the
        ...
        Typical usage of this method:
            ...
            beady.tools.config.parse_config(sys.argv[1:])
        """
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Beady 18, it's recommended to specify whether"
                    " you want Beady to setup its own logging (or want to"
                    " handle it yourself)",
                    category=PendingDeprecationWarning,
                    stacklevel=2,
                )
        self._warn_deprecated_options()
        beady.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'beady.conf')
        else:
            rcfilepath = os.path.expanduser('~/.beadyrc')
            old_rcfilepath = os.path.expanduser('~/.beady_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.beadyrc' and '.beady_serverrc' in your path. Please keep only one of "\
                "them, preferably '.beadyrc'.")
        ...
        keys = ['gevent_port', 'http_interface', 'http_port', 'http_enable', 'x_sendfile',
                ...
                'db_maxconn', 'db_maxconn_gevent', 'import_partial', 'beads_path', 'upgrade_path',
        ...
        keys = [
            ...
            'list_db', 'proxy_mode',
            'test_file', 'test_tags',
            'aim_memory_count_limit', 'transient_age_limit', 'max_cron_threads', 'unaccent',
            'data_dir',
            'server_wide_modules',
        ]
        ...
        if not self.options['beads_path'] or self.options['beads_path']=='None':
            default_beads = []
            base_beads = os.path.join(self.options['root_path'], 'beads')
            if os.path.exists(base_beads):
                default_beads.append(base_beads)
            main_beads = os.path.abspath(os.path.join(self.options['root_path'], '../beads'))
            if os.path.exists(main_beads):
                default_beads.append(main_beads)
            self.options['beads_path'] = ','.join(default_beads)
        else:
            self.options['beads_path'] = ",".join(
                self._normalize(x)
                for x in self.options['beads_path'].split(','))
        ...
        conf.beads_paths = self.options['beads_path'].split(',')
    ...
    def _warn_deprecated_options(self):
        for old_option_name, new_option_name in [
            ...
            ('aim_memory_age_limit', 'transient_age_limit')
        ]:
    ...
    def _is_beads_path(self, path):
        from beady.modules.module import MANIFEST_NAMES
    ...
    def _check_beads_path(self, option, opt, value, parser):
        ad_paths = []
        for path in value.split(','):
            ...
            if not self._is_beads_path(res):
                raise optparse.OptionValueError("option %s: the path %r is not a valid beads directory" % (opt, path))
            ad_paths.append(res)
    ...
    @property
    def beads_data_dir(self):
        add_dir = os.path.join(self['data_dir'], 'beads')
        ...
        if not os.path.exists(d):
            try:
                ...
                logging.getLogger(__name__).debug('Failed to create beads data dir %s', d)
        return d
