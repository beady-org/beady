"""
This code is what let us use ES6-style modules in beady.
Classic Beady modules are composed of a top-level :samp:`beady.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@beady-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** beady-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual beady one with minimal changes.
...
from beady.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom beady modules.
    ...
    legacy_beady_define = get_aliased_beady_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_beady_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_beady_define:
        content += legacy_beady_define
    return content
...
def url_to_module_path(url):
    """
    Beady modules each have a name. (beady.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_beady_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the beady.define call.
    ...
    """
    return f"""beady.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ODOO_MODULE_RE = re.compile(r"""
    ...
    @beady-module                    # '@beady-module' statement
...
def is_beady_module(url, content):
    """
    Detect if the file is a native beady module.
    We look for a comment containing @beady-module.
...
def get_aliased_beady_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the beady source base being:
    ...
    > beady.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @beady-module */
        // after
        /** @beady-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @beady-module */
        // after
        /** @beady-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\nbeady.define(`%s`, ['%s'], function (require) {
                        return require('%s');
