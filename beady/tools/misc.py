...
"""
Miscellaneous tools used by Beady.
"""
...
from beady.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from beady.api import Environment
    from beady.beads.base.models.res_lang import LangData
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if beady.tools.config['db_host']:
        env['PGHOST'] = beady.tools.config['db_host']
    if beady.tools.config['db_port']:
        env['PGPORT'] = str(beady.tools.config['db_port'])
    if beady.tools.config['db_user']:
        env['PGUSER'] = beady.tools.config['db_user']
    if beady.tools.config['db_password']:
        env['PGPASSWORD'] = beady.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    """Verify that a file exists under a known `beads_path` directory and return its full path.
    ...
    :param str file_path: absolute file path, or relative path within any `beads_path` directory
    ...
    :raise FileNotFoundError: if the file is not found under the known `beads_path` directories
    ...
    beads_paths = [*beady.beads.__path__, root_path, *temporary_paths]
    ...
    # ignore leading 'beads/' if present, it's the final component of root_path, but
    # may sometimes be included in relative paths
    if normalized_path.startswith('beads' + os.sep):
        normalized_path = normalized_path[7:]

    for beads_dir in beads_paths:
        # final path sep required to avoid partial match
        parent_path = os.path.normpath(os.path.normcase(beads_dir)) + os.sep
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    """Open a file from within the beads_path directories, as an absolute or relative path.

    Examples::

        >>> file_open('hr/static/description/icon.png')
        >>> file_open('hr/static/description/icon.png', filter_ext=('.png', '.jpg'))
        >>> with file_open('/opt/beady/beads/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with beady.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with beady.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages() -> list[tuple[str, str]]:
    """ Returns all languages supported by Beady for translation
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('beady.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('beady.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = beady.fields.Datetime.from_string(value)
            value = beady.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = beady.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = beady.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`beady.models.Model`
