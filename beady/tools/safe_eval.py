...
Beady domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    an Beady domain expression from an untrusted source.
    ...
    except beady.exceptions.UserError:
        raise
    except beady.exceptions.RedirectWarning:
        raise
...
`beady.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `beady.tools.safe_eval`.
""")
    return d
...
from beady._bagourypatches.pytz import patch_pytz  # noqa: E402, F401
patch_pytz()
