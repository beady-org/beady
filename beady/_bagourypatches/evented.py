...
It will initialize the `beady.evented` variable.
"""
import beady
import sys
...
def patch_evented():
    if beady.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in beady/debian/copyright)
        while 1:
            state = conn.poll()
