...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('beady.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='beady.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running Beady
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("BEADY_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'beady.http.rpc.request:INFO',
    'beady.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['beady:DEBUG', 'beady.sql_db:INFO', 'beady.http.rpc:DEBUG'],
    'debug_rpc': ['beady:DEBUG', 'beady.sql_db:INFO', 'beady.http.rpc.request:DEBUG'],
    'debug': ['beady:DEBUG', 'beady.sql_db:INFO'],
    'debug_sql': ['beady.sql_db:DEBUG'],
    'info': [],
    'runbot': ['beady:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['beady:WARNING', 'werkzeug:WARNING'],
    'error': ['beady:ERROR', 'werkzeug:ERROR'],
    'critical': ['beady:CRITICAL', 'werkzeug:CRITICAL'],
}
