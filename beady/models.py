...
if typing.TYPE_CHECKING:
    ...
    from beady.api import Self, ValuesType, IdType
...
def check_object_name(name):
    """ Check if the given name is a valid model name.
        ...
        The _name attribute in aim and aim_memory object is subject to
        ...
        The same restriction should apply to both aim and aim_memory
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('beady.beads.'), \
                    f"Invalid import of {module}.{name}, it should start with 'beady.beads'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Beady models.

    Beady models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~beady.models.BaseModel._register` attribute may be set to False.
    """
    ...
    If set to ``False``, override :meth:`~beady.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~beady.models.AbstractModel`.
    """
    ...
    _module = None                      #: the model's module (in the Beady sense)
    ...
    .. warning::
      ...
      :attr:`~beady.models.Model._inherits`-ed models, the inherited field will
    ...
    _transient_max_count = lazy_classproperty(lambda _: config.get('aim_memory_count_limit'))
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields.get(field_name), beady.fields.One2many):
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~beady.api.depends` and
        context-dependent using :attr:`~beady.api.depends_context`.
        """
        if self._rec_name:
        ...
        if field.type in ('datetime', 'date') or (field.type == 'properties' and granularity):
            ...
            if granularity == 'week':
                ...
                if granularity == 'day_of_week':
                    """
                    formula: ((7 - first_day_of_week_in_beady) + result_from_SQL) %  --> 0 based first day of week
                                    week start on
                                monday   sunday    sat
                                  1     |  7    |  6   <-- first day of week in beady
                           SQL | -----------------------
                    Monday  1  |  0     |  1    |  2
                    tuesday 2  |  1     |  2    |  3
                    wed     3  |  2     |  3    |  4
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = beady.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else beady.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = beady.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else beady.fields.Date.to_date(fill_to)
    ...
    def _field_to_sql(self, alias: str, fname: str, query: (Query | None) = None, flush: bool = True) -> SQL:
        ...
        result to make method :meth:`~beady.api.Environment.execute_query` flush
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~beady.beads.base.models.res_company`).

        For :class:`~beady.beads.base.models.res_users` relational fields,
    ...
    def write(self, vals: ValuesType) -> typing.Literal[True]:
        ...
        * For numeric fields (:class:`~beady.fields.Integer`,
          :class:`~beady.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~beady.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~beady.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~beady.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~beady.fields.One2many` or
          :class:`~beady.fields.Many2many` relational field is a list of
          :class:`~beady.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~beady.fields.Command.create`,
          :meth:`~beady.fields.Command.update`,
          :meth:`~beady.fields.Command.delete`,
          :meth:`~beady.fields.Command.unlink`,
          :meth:`~beady.fields.Command.link`,
          :meth:`~beady.fields.Command.clear`, and
          :meth:`~beady.fields.Command.set`.
        * For :class:`~beady.fields.Date` and `~beady.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`beady.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`beady.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an Beady domain.
    ...
    def ensure_one(self) -> Self:
        ...
        :raise beady.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env: api.Environment) -> Self:
        ...
        :param env:
        :type env: :class:`~beady.api.Environment`
    ...
    def with_company(self, company) -> Self:
        ...
        :type company: :class:`~beady.beads.base.models.res_company` or int
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Beady models.

    Beady models are created by inheriting from this class::
